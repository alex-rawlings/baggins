...
\begin{equation}\label{eq:dehnen}
    \rho_\star(r) = \frac{M_\star}{2\pi} \frac{a}{r(r+a)^{(4-\gamma)}}
\end{equation}
...
We then extract the evolved systems, and use these as the progenitors for all subsequent merger configurations. 
...
The progenitors are evolved through the \textit{parent phase} from $t(r_0)=0$ with \gadget until the SMBHs form a bound binary at $t_\text{stop}$, with a stellar softening of $\epsilon_\star=10\,\text{pc}$ and integration tolerances \drafting{check}.
...
The Gaussian distributions are characterised by $\mathcal{N}_\text{pos}\sim(\mu=0\,\text{pc}, \sigma^2=100\,\text{pc})$, and $\mathcal{N}_\text{vel}\sim(\mu=0\,\text{km\,s}^{-1}, \sigma^2=100\,\text{km\,s}^{-1})$ for each progenitor \drafting{should we show a figure of this?}. 
...
Additionally, we lower the stellar softening to $\epsilon_\star=3.5\,\text{pc}$ and tighten the integration tolerances to \drafting{check} to optimise the run for \ketju.
