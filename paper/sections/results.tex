...
We compare the latent parameter distributions in \autoref{fig:compareAC} for the AC merger class\footnote{The merger family A-C-3.0-1.0 did not have enough \child samples (3 from 10) from which to construct the likelihood function, and thus is omitted here.}, with line colour corresponding to the first pericentre distance of the merger family, $r_\mathrm{peri}/R_{\mathrm{vir},1}$. 
We find no convincing evidence for an influence of first pericentre distance on the resulting projected mass density profiles, with the distributions of latent parameters consistent between all families in the AC progenitor class. 
Whilst we purposely avoid point estimates on our latent parameter distributions, we report the median values here to give a sense of scale. 
...
\begin{figure*}
    ...
    \caption{\drafting{Comparing the latent parameters for different members of the same progenitor class. Here shown is those families part of the AC merger class. There is no significant difference in latent parameter values with increasing rperi. TODO do this same plot for each merger class.}}
    \label{fig:compareAC}
\end{figure*}
