...
parameters {
    ...
    real<lower=0.0> r_b;
    real<lower=0.0> Re;
    real<lower=0.0> log10_I_b;  // sample ln(surf. dens.) directly
    real<lower=0.0> g;
    real<lower=0.0> n;
    real<lower=0.0> a;
...
transformed parameters {
    ...
    log10_surf_rho_calc = log10_I(N_tot, R, log10_I_b, g, a, r_b, Re, n);
}
...
model{
    ...
    target += normal_lpdf(log10_I_b | 0.0, 20.0);
...
generated quantities {
    ...
    for(i in 1:N_tot){
        Ri[1] = R[i];
        Ii = normal_rng(log10_I(1, Ri, log10_I_b, g, a, r_b, Re, n), err);
