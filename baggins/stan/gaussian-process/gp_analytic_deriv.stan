functions {
    ...
    // Predict derivative process dy/dx
    vector gp_deriv_pred_rng(array[] real x2,
                             vector y1,
                             array[] real x1,
                             real alpha,
                             real rho,
...

      {
        ...
        matrix[N1, N1] L_K;
        vector[N1] K_div_y1;
        matrix[N1, N2] dk_x1_x2;    // derivative cross-cov
        vector[N2] df2_mu;
        matrix[N2, N2] cov_df2;
        matrix[N2, N2] diag_delta;
        ...
        // Compute cross-covariance between f(x1) and f'(x2)
        for (n1 in 1:N1) {
          for (n2 in 1:N2) {
            real d = x1[n1] - x2[n2];
            real base = square(alpha) * exp(-0.5 * square(d / rho));
            dk_x1_x2[n1, n2] = -(d / square(rho)) * base;
          }
        }

        // Mean of derivative GP
        df2_mu = dk_x1_x2' * K_div_y1;

        // Compute covariance of derivative GP
        // Cov(f'(x), f'(x')) = ∂²/∂x∂x' k(x, x')
        cov_df2 = rep_matrix(0, N2, N2);
        for (i in 1:N2) {
          for (j in 1:N2) {
            real d = x2[i] - x2[j];
            real base = square(alpha) * exp(-0.5 * square(d / rho));
            cov_df2[i, j] = base * (
              (1 / square(rho)) - (square(d) / pow(rho, 4))
            );
          }
        }

        diag_delta = diag_matrix(rep_vector(delta, N2));
        df2 = multi_normal_rng(df2_mu, cov_df2 + diag_delta);
      }
      return df2;
    }
}
...
generated quantities {
    ...
    vector[N2] f2;
    vector[N] yR;
    vector[N2] df2;   // derivative predictions
    vector[N] powy;
    vector[N2] powdy;
    ...
    // derivative at prediction points
    df2 = gp_deriv_pred_rng(x2R, y1R, x1R, alpha, rho, sigma, delta);

    // rescale data back
    vector[N] y = yR * y1_std + y1_mean;
    // rescale derivative back: dy/dx = (y_std/x_std) * df2
    vector[N2] dydx = df2 * (y1_std / x1_std);
