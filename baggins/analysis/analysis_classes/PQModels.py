...
class _PQModelBase(HierarchicalModel_2D):
    ...
    def extract_data(self, dir, pars):
        ...
        for f in dir:
            ...
            try:
                idx = hmq.get_idx_in_vec(
                    np.nanmedian(hmq.hardening_radius), hmq.semimajor_axis
                )
            except ValueError:
                _logger.warning(
                    f"No data prior to merger! The requested semimajor axis value is {np.nanmedian(hmq.hardening_radius)}, semimajor_axis attribute is: {hmq.semimajor_axis}. This run will not form part of the analysis."
                )
                continue
            except AssertionError:
                _logger.warning(
                    f"Trying to search for value {np.nanmedian(hmq.hardening_radius)}, but an AssertionError was thrown. The array bounds are {min(hmq.semimajor_axis)} - {max(hmq.semimajor_axis)}. This run will not form part of the analysis."
                )
                continue
