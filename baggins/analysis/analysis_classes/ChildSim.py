...
class ChildSim(BHBinary, ChildSimData):
    def __init__(self, paramfile, perturbID, apfile) -> None:
        ...
        apfile : str
            see BHBinary
        """
        _logger.info("> Determining binary quantities")
        ...
        for k, v in afv.radial_edges.items():
            ...
            self.radial_bin_centres[k] = pygad.UnitArr(
                get_histogram_bin_centres(v), units=self.radial_edges[k].units
            )
        ...
        _logger.info("> Determining particle counts")
        ...
        _logger.info("> Determining stellar velocity dispersion")
        self.relaxed_stellar_velocity_dispersion = np.nanstd(
            self.main_snap.stars[self.galaxy_radius_mask]["vel"], axis=0
        )

        # projected effective radius, velocity dispersion in 1Re, and density
        _logger.info("> Determining projected quantities")
        (
            self.relaxed_effective_radius,
            self.relaxed_stellar_velocity_dispersion_projected,
            self.relaxed_density_profile_projected,
        ) = self._get_projected_quantities()

        # DM fraction within 1Re
        _logger.info("> Determining inner DM fraction")
        self.relaxed_inner_DM_fraction = inner_DM_fraction(
            self.main_snap, Re=self.relaxed_effective_radius["estimate"]
        )

        # 3D half mass radius
        _logger.info("> Determining half mass radius")
        self.relaxed_half_mass_radius = pygad.analysis.half_mass_radius(
            self.main_snap.stars[self.galaxy_radius_mask]
        )

        # total stellar mass of the remnant
        _logger.info("> Determining total stellar mass")
        self.total_stellar_mass = pygad.UnitScalar(
            self.main_snap.stars["mass"][0]
            * len(self.main_snap.stars[self.galaxy_radius_mask]),
            units=self.main_snap["mass"].units,
        )

        # 3D density profile of relaxed remnant
        _logger.info("> Determining 3D density profile")
        self.relaxed_density_profile = self._dens_prof_helper()

        # relaxed core parameters
        _logger.info("> Determining core fit parameters")
        try:
            self.relaxed_core_parameters = fit_Terzic05_profile(
                self.radial_bin_centres["stars"],
                self.relaxed_density_profile["stars"],
                self.relaxed_effective_radius["estimate"],
                max_nfev=1000,
            )
        except RuntimeError:
            self.add_to_log("Core-fit parameters could not be determined! Skipping...")
            self.relaxed_core_parameters = {
                "rhob": np.nan,
                "rb": np.nan,
                "n": np.nan,
                "g": np.nan,
        ...
        _logger.info("> Determining triaxiality parameters at merger")
        (
            self.relaxed_triaxiality_parameters,
            self.binding_energy_bins,
        ) = self._triaxial_helper()

        # IFU data at time when a = a_h
        _logger.info("> Determining IFU data at time of a=a_h")
        self.ifu_map_ah = self.create_ifu_map(t=self.r_hard_time)

        # IFU data at merger
        _logger.info("> Determining IFU data at time of merger / last snap")
        ...
        _logger.info("> Determining time series data")
        (
            self.snapshot_times,
            self.bh_binary_watershed_velocity,
            self.stellar_shell_inflow_velocity,
            self.ang_mom_diff_angle,
            self.loss_cone,
            self.stars_in_loss_cone,
            self.ang_mom,
            self.num_escaping_stars,
        ) = self._get_time_series_data(R=self.shell_radius)

        # velocity anisotropy of remnant as a function of radius
        _logger.info("> Determining beta profile")
    ...
    def main_snap(self):
        if self.merged():
            self.merged_or_last_idx = snap_num_for_time(
                self.snaplist, self.merged.time, method="floor"
            )
        else:
            self.merged_or_last_idx = -1
        _logger.info(f"Merger remnant snapshot is number {self.merged_or_last_idx}")
        ...
        if not np.any(xcom_None_bool) and not np.any(vcom_None_bool):
            ...
            if np.any(
                np.abs(np.diff(list(xcom.values()), axis=0))
                > self.com_consistency["pos"] / 1e3
            ):
                warnings.warn(msg.format("position"))
                self.add_to_log(msg.format("position"))
                self.relaxed_remnant_flag = False
            if np.any(
                np.abs(np.diff(list(vcom.values()), axis=0))
                > self.com_consistency["vel"]
            ):
        ...
        _logger.info("> Determining virial information")
        self.virial_info = {"radius": None, "mass": None}
        (
            self.virial_info["radius"],
            self.virial_info["mass"],
        ) = pygad.analysis.virial_info(snap)
        virial_mask = pygad.BallMask(self.virial_info["radius"])
        s = (
            np.linalg.norm(pygad.analysis.center_of_mass(snap[virial_mask]))
            / self.virial_info["radius"]
        )
    ...
    def _dens_prof_helper(self):
        ...
        rhos["stars"] = pygad.analysis.profile_dens(
            self.main_snap.stars[self.galaxy_radius_mask],
            "mass",
            r_edges=self.radial_edges["stars"],
        )
        # 3D density profile for DM
        rhos["dm"] = pygad.analysis.profile_dens(
            self.main_snap.dm, "mass", r_edges=self.radial_edges["dm"]
        )
        # 3D density profile for ALL
        rhos["all"] = pygad.analysis.profile_dens(
            self.main_snap, "mass", r_edges=self.radial_edges["all"]
        )
        return rhos

    def _get_projected_quantities(self):
        Re, vsig, rho = projected_quantities(
            self.main_snap[self.galaxy_radius_mask], r_edges=self.radial_edges["stars"]
        )
        return list(Re.values())[0], list(vsig.values())[0], list(rho.values())[0]

    def _triaxial_helper(self):
        ratios = dict(ba=np.full(20, np.nan), ca=np.full(20, np.nan))
        try:
            energy_mask_gen = get_binding_energy_mask(
                self.main_snap[self.galaxy_radius_mask], family="stars"
            )
            i = 0
            while True:
                try:
                    this_energy_mask, energy_bins, self._energy_units = next(
                        energy_mask_gen
                    )
                except StopIteration:
                    break
                ratios["ba"][i], ratios["ca"][i] = get_galaxy_axis_ratios(
                    self.main_snap[self.galaxy_radius_mask], bin_mask=this_energy_mask
                )
                i += 1
    ...
    def _get_time_series_data(self, R=30, tspan=0.1):
        ...
        bound_snap_idx = snap_num_for_time(
            self.snaplist, self.orbit_params["t"][0] / myr
        )
        last_snap_idx = snap_num_for_time(
            self.snaplist, self.orbit_params["t"][-1] / myr
        )
        ...
        bh_stars_J = {
            "bh": np.full((len(t), 3), np.nan),
            "stars": np.full((len(t), 3), np.nan),
        }
        ...
        for i, j in enumerate(np.arange(bound_snap_idx, last_snap_idx + 1)):
            ...
            try:
                ...
                _a = np.nanmedian(
                    self.orbit_params["a_R"][idx - delta_idx : idx + delta_idx]
                )
                _e = np.nanmedian(
                    self.orbit_params["e_t"][idx - delta_idx : idx + delta_idx]
                )
                ...
                star_J_mag = pygad.utils.geo.dist(
                    snap.stars[self.galaxy_radius_mask]["angmom"]
                )
            ...
            (
                ...
                bh_stars_J["bh"][i, :],
                bh_stars_J["stars"][i, :],
            ) = angular_momentum_difference_gal_BH(snap, mask=self.galaxy_radius_mask)
            # determine the number of hypervelocity stars
            num_vescs[i], prev_vescs = count_new_hypervelocity_particles(
                snap, prev=prev_vescs
            )
            # save memory
            snap.delete_blocks()
        ...
        if t[1] - t[0] > tspan:
            warnings.warn(
                f"ketjugw quantities averaged over an interval {tspan:.2f} Myr that is larger than the snapshot interval {t[1]-t[0]:.2f}!"
            )
        return (
            pygad.UnitArr(t, units="Myr"),
            pygad.UnitArr(w, units="km/s"),
            v,
            theta,
            pygad.UnitArr(J_lc, units=J_unit),
            num_J_lc,
            bh_stars_J,
            num_vescs,
        )
    ...
    def _beta_r_helper(self):
        vspherical = spherical_components(
            self.main_snap.stars[self.galaxy_radius_mask]["pos"],
            self.main_snap.stars[self.galaxy_radius_mask]["vel"],
        )
        beta_r, radbins, bincount = beta_profile(
            self.main_snap.stars[self.galaxy_radius_mask]["r"], vspherical, 0.05
        )
    ...
    def plot(self):
        warnings.warn(
            "This method is not available for this class. If you wish to plot the BH Binary orbital parameters, use the binary_plot() method."
        )
    ...
    def create_ifu_map(self, t=None, r=None, idx=None):
        if idx is None and t is not None:
            ...
            raise RuntimeError(
                "One of t (time) or idx (snapshot number) must be specified!"
            )
    ...
    def make_hdf5(self, fname, exist_ok=False):
        ...
        with h5py.File(fname, mode="w") as f:
            ...
            self._add_attr(
                f["/galaxy_properties/stellar_shell_inflow_velocity"],
                "shell_radius",
                self.shell_radius,
            )
            self._add_attr(
                f["/galaxy_properties/binding_energy_bins"], "units", self._energy_units
            )
