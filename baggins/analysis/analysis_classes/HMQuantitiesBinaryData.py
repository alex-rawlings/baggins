...
class HMQuantitiesBinaryData(HMQuantitiesSingleData):
    ...
    def idx_finder(self, val, vec):
        ...
        try:
            ...
            _logger.warning(
                f"No data prior to merger! The requested semimajor axis value is {val}, semimajor_axis attribute is: {vec}. This run will not form part of the analysis."
            )
            status = False
            idx = -9999
        except AssertionError:
            _logger.warning(
                f"Trying to search for value {val}, but an AssertionError was thrown. The array bounds are {min(vec)} - {max(vec)}. This run will not form part of the analysis."
            )
            status = False
            idx = -9999
        return status, idx
