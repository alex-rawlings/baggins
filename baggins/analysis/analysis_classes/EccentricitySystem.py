...
__all__ = ["EccentricitySystem", "EccentricitySystemScanner"]
...
class EccentricitySystem:
    def __init__(
        self, m1, m2, ellipticity=None, rho=None, stellar_sigma=None, rmax=None
    ):
        """
        ...
        self._rho = rho
        self._stellar_sigma = stellar_sigma * self._km_per_s
        self.rmax = rmax
    ...
    def m1(self, m):
        self._m1 = m
        for k in self._cached_needs_updating:
    ...
    def argument_of_periapsis(self):
        ...
        evec = (
            np.cross(self.v[:, i], [0, 0, h])[:2] / (self._G * self.mbin)
            - self.x[:, i] / r[i]
        )
    ...
    @cached_property
    def b90(self):
        return 2 * self._G * self.m1 / self._stellar_sigma**2
    ...
    @cached_property
    def a_hard(self):
        return self._G * self.reduced_mass / (4 * self._stellar_sigma**2)

    @cached_property
    def r_infl(self):
        return np.cbrt(self.mbin / (self._rho * (4 / 3 * np.pi)))
    ...
    def Avec(self):
        ...
        A1 = (
            ...
            * (
                ...
                - 1
                / (2 * self.ellipticity)
                * np.log((1 + self.ellipticity) / (1 - self.ellipticity))
            )
        )
        A3 = (
            2
    ...
    def ellipsoid_accel(self, x):
        ...
        """
        return -2 * np.pi * self._G * self._rho * self.Avec * x
    ...
    def dynamical_friction(self, v):
        ...
        def _df_taylor(m):
            ...
            return -(
                ...
                * self._rho
                * self.logL
                / (3 * np.sqrt(2) * self._stellar_sigma**3)
            )

        def _df_general(m):
            v_single, v_single_norm = _set_vel(m, v)
            X = v_single_norm / (np.sqrt(2) * self._stellar_sigma)
            return -(
                4
                * np.pi
                * self._G**2
        ...
        df1 = (
            _df_taylor(self.m1)
            if v1_norm / self._stellar_sigma < 1e-3
            else _df_general(self.m1)
        )
        df2 = (
            _df_taylor(self.m2)
            if v2_norm / self._stellar_sigma < 1e-3
            else _df_general(self.m2)
        )
        return df1 + df2
    ...
    def accel(self, x, v):
        ...
        """
        return (
            -self._G * self.mbin / np.linalg.norm(x, axis=0) ** 3 * x
            + self.ellipsoid_accel(x * self.m1 / self.mbin)
            + self.ellipsoid_accel(x * self.m2 / self.mbin)
    ...
    def orbital_energy(self, x, v):
        ...
        return 0.5 * v2 - self._G * self.mbin / r
    ...
    def integrate(self, args):
        ...
        ts = np.concatenate(
            (
                [0],
                np.linspace(0.5 * r / v, 1.5 * r / v, 50),
                np.linspace(1.5 * r / v, tmax, 500)[1:],
            )
        )
    ...
    def calculate_b_theta_e_curves(self, v0s, bs, r0=None):
        ...
        with ProcessPoolExecutor() as executor:
            futures = {
                executor.submit(self.integrate, x): i
                for i, x in enumerate(
                    zip(Bs.ravel(), V0s.ravel(), itertools.repeat(r0))
                )
            }
            for future in tqdm(
    ...
    def plot_theta_ecc_curve(self, v0, ax=None, shift=0):
        ...
        ax.plot(
            np.degrees(self.theta[i]) + shift,
            self.ecc[i],
            label=f"$v_0 = {self.v0s[i]:3.0f}" r"/\mathrm{km\,s^{-1}}$",
        )
        ax.legend()
        return ax
    ...
    def plot_theta_ecc_curve_all_velocities(self, ax=None, shift=0):
        ...
        lines = LineCollection(
            list(map(np.column_stack, zip(np.degrees(self.theta) + shift, self.ecc))),
            array=self.v0s,
            cmap="cividis",
            lw=2,
        )
...
class EccentricitySystemScanner:
    def __init__(self, ecc_sys, ellipticity, rmax, v0s, bs, r0=None, data_path=""):
        ...
        if isinstance(ellipticity, (list, tuple)):
            ...
            raise TypeError(
                f"Unrecognised input {type(ellipticity)} for parameter 'ellipticity'!"
            )
    ...
    def scan(self, exist_ok=True):
        ...
        for ellipticity, rmax in itertools.product(self.ellipticity, self.rmax):
            ...
            _logger.info(f"Scanning system: {self.ecc_sys}")
