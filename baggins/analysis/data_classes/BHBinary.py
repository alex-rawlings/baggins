...
from baggins.general.pygad_helper import snap_num_for_time
...
from baggins.general.general import xval_of_quantity
from baggins.general.pygad_helper import convert_gadget_time
from baggins.mathematics.statistics import iqr
from baggins.plotting.specific_plots import binary_param_plot
from baggins.utils.param_io import read_parameters
from baggins.utils.data_handling import get_ketjubhs_in_dir, get_snapshots_in_dir
from baggins.env_config import date_format, username, _cmlogger
...
class BHBinary(BHBinaryData):
    def __init__(self, paramfile, perturbID, apfile) -> None:
        ...
        if self.merger_pars["file_locations"]["perturb_sub_dir"] is None:
            ...
            data_path = os.path.join(
                self.merger_pars["file_locations"]["save_location"], perturbID, "output"
            )
            ...
            data_path = os.path.join(
                self.merger_pars["calculated"]["full_save_location"],
                self.merger_pars["file_locations"]["perturb_sub_dir"],
                perturbID,
                "output",
            )
        ...
        self.param_estimate_e_quantiles = self.analysis_pars["bh_binary"][
            "eccentricity_quantiles_for_estimates"
        ]

        # set the properties
        snap_idx = snap_num_for_time(
            self.snaplist,
            self.orbit_params["t"][0] / myr
            + convert_gadget_time(
                pygad.Snapshot(self.snaplist[0], physical=True), "Myr"
            ),
            method="ceil",
        )
        ...
        self.binary_spin_flip = np.any(
            np.abs(np.diff(np.sign(self.orbit_params["plane_normal"][:, 2]))) > 0
        )
        ...
        try:
            ...
            self.r_infl_time_idx = self._get_idx_in_vec(
            ...
            self.formation_ecc_spread = iqr(
                self.orbit_params["e_t"][: self.r_infl_time_idx]
            )
        except ValueError:
        ...
        self.r_bound = list(enclosed_mass_radius(snap, mass_frac=0.1).values())[
            0
        ].in_units_of("pc")
        ...
        try:
            ...
            self.r_bound_time_idx = self._get_idx_in_vec(
                self.r_bound_time, self.orbit_params["t"] / myr
            )
        ...
        try:
            ...
            self.r_hard_time_idx = self._get_idx_in_vec(
        ...
        self.analytical_tspan = pygad.UnitScalar(
            self.orbit_params["t"][self.a_more_Xpc_idx] / myr - self.r_hard_time,
            units="Myr",
        )

        # ratio of G * density / inner velocity dispersion
        self.G_rho_per_sigma = get_G_rho_per_sigma(
            self.snaplist, self.r_hard_time, extent=self.r_infl
        )

        # semimajor axis hardening constant
        self.H = linear_fit_get_H(
            self.orbit_params["t"] / ketjugw.units.yr,
            self.orbit_params["a_R"] / ketjugw.units.pc,
            self.r_hard_time * 1e6,
            self.analytical_tspan * 1e6,
            self.G_rho_per_sigma,
        )

        # eccentricity constant
        self.K = linear_fit_get_K(
            self.orbit_params["t"] / ketjugw.units.yr,
            self.orbit_params["e_t"],
            self.r_hard_time * 1e6,
            self.analytical_tspan * 1e6,
            self.H,
            self.G_rho_per_sigma,
    ...
    def _get_time_for_r(self, r, desc="r"):
        ...
        try:
            tval = xval_of_quantity(
                r,
                self.orbit_params["t"] / myr,
                self.orbit_params["a_R"] / ketjugw.units.pc,
                xsorted=True,
            )
            ...
            if r > self.orbit_params["a_R"][0] / ketjugw.units.pc:
                _logger.warning(
                    f"Time of {desc} cannot be estimated from the data, as it occurs before the binary is bound! This point will be omitted from further analysis and plots."
                )
            elif r < self.orbit_params["a_R"][-1] / ketjugw.units.pc:
                _logger.warnings(
                    f"Time of {desc} cannot be estimated from the data, as it occurs after the last available data point! This point will be omitted from further analysis and plots."
                )
            else:
    ...
    def compute_predict_orbital_params(self, q, idxs=None):
        ...
        t, a, e = analytic_evolve_peters_quinlan(
            a0,
            e0,
            self.orbit_params["t"][idxs[0]],
            self.orbit_params["t"][-1],
            self.orbit_params["m0"][0],
            self.orbit_params["m1"][0],
            self.G_rho_per_sigma * self.H,
            self.K,
        )
        t /= myr
        a /= ketjugw.units.pc
    ...
    def plot(self, ax=None, add_radii=True, add_op_estimates=True, **kwargs):
        ...
        if add_radii:
            ax[0].scatter(
                self.r_infl_time + self.time_offset,
                self.r_infl,
                zorder=10,
                label=r"$r_\mathrm{inf}$",
            )
            ax[0].scatter(
                self.r_hard_time + self.time_offset,
                self.r_hard,
                zorder=10,
                label=r"$a_\mathrm{h}$",
            )
            ax[0].axvspan(
                self.r_hard_time + self.time_offset,
                self.r_hard_time + self.analytical_tspan + self.time_offset,
                color="tab:red",
                alpha=0.4,
                label="H calculation",
            )
            sc = ax[0].scatter(
                self.gw_dominant_semimajoraxis["t"] + self.time_offset,
                self.gw_dominant_semimajoraxis["a"],
                zorder=10,
                label=r"$a_\mathrm{GR}$",
            )
            ax[0].axhline(
                self.gw_dominant_semimajoraxis["a"], ls=":", c=sc.get_facecolors()
            )
            agr_low, _ = self.predict_gw_dominant_semimajoraxis(
                self.param_estimate_e_quantiles[0]
            )
            agr_high, _ = self.predict_gw_dominant_semimajoraxis(
                self.param_estimate_e_quantiles[-1]
            )
            ax[0].axhspan(agr_low, agr_high, facecolor=sc.get_facecolors(), alpha=0.4)
        if add_op_estimates:
            for i, (k, q) in enumerate(
                zip(
                    self.predicted_orbital_params["a"].keys(),
    ...
    def _make_hdf5_helper(self, f):
        ...
        self._add_attr(
            f["/bh_binary/predicted_orbital_params"],
            "quantiles",
            self.param_estimate_e_quantiles,
        )
