...
__all__ = [
    ...
    "determine_merger_timescale",
    "get_hard_timespan",
    "find_idxs_of_n_periods",
    "impact_parameter",
    "move_to_centre_of_mass",
    "deflection_angle",
    "first_major_deflection_angle",
]
...
def get_binary_before_bound(ketju_file, tol=1e-15):
    ...
    try:
        ...
        if np.all(energy_mask):
            _logger.warning(
                "Binary system has not become bound! We will use all data from the BH particles."
            )
            bound_idx = -1
            bound_state = "never"
...
def _do_linear_fitting(t, y, t0, tspan, return_idxs=False):
    ...
    if t0 + tspan >= t[-1]:
        tfidx = -1
        _logger.warning(
            "Analytical fit to binary evolution done to the end of the time data -> proceed with caution!"
        )
    # assume dy/dt is a approx. linear
    popt, pcov = scipy.optimize.curve_fit(
        lambda x, a, b: a * x + b, t[t0idx:tfidx], y[t0idx:tfidx]
    )
    if return_idxs:
...
def analytic_evolve_peters_quinlan(a0, e0, t0, tf, m1, m2, HGp_s, K, N_orbits=2):
    ...
    ap, ep, _, _, tp = ketjugw.orbit.peters_evolution(
        a0=a0,
        e0=e0,
        m1=m1,
        m2=m2,
        ts=(t0, propagate_time, N_orbits),
        ext_derivs=quinlan_derivatives,
    )
    return tp, ap, ep


def determine_merger_timescale(
    a0, e0, t0, tf, m1, m2, HGp_s, K, atol=1e-3, N_orbits=2, etol=1e-3, nrep=1000
):
    """
    ...
    while af > atol and ef > etol and n < nrep:
        tp, ap, ep = analytic_evolve_peters_quinlan(
            a0=a0,
            e0=e0,
            t0=t0,
            tf=tf,
    ...
    try:
        ...
        _logger.exception(
            f"Merger timescale has not converged: system has final a={af:.3e} and e={ef:.3e}",
            exc_info=True,
        )
        raise
...
def find_idxs_of_n_periods(
    tval, tarr, sep, num_periods=1, max_iter=100, strict_mode=False
):
    """
    ...
    if num_periods % 2 == 0:
        _logger.warning(
            f"Only odd values of <num_periods> implemented, we will search for {num_periods+1} periods."
        )
    end_idxs = [0, 0]
    ...
    while not found_peaks:
        idxs = np.r_[
            max(0, idx - 10 * multiplier) : min(max_idx, idx + 10 * multiplier)
        ]
        ...
        if len(peaks) > 2 * num_periods or iter_n == max_iter:
            if iter_n == max_iter:
                _logger.error(
                    f"Maximum number of iterations ({max_iter}) reached, and only {int(len(peaks)/2)}/{num_periods} have been found!"
                )
                try:
            ...
            _idx0 = (
                np.where(peaks_rel < 0, peaks_rel, -np.inf).argmax() - num_periods // 2
            )
            if _idx0 < 0:
                _idx1 = -_idx0
                _idx0 = 0
                try:
                    assert not strict_mode
                    _logger.warning(
                        f"Not enough complete orbits before desired time! Number of orbits used will be truncated to {_idx1}"
                    )
                except AssertionError:
                    _logger.exception(
                ...
                _idx1 = min(
                    np.where(peaks_rel >= 0, peaks_rel, np.inf).argmin()
                    + num_periods // 2,
                    len(peaks_rel) - 1,
                )
            ...
            try:
                ...
                _logger.exception(
                    f"Period start index is greater than or equal to the period end index: {end_idxs}. An error has occurred in the calculation! Search mutlitplier was {multiplier}, central index was {idx}, and {len(peaks)} peaks have been identified.",
                    exc_info=True,
                )
                raise
        else:
...
def move_to_centre_of_mass(bh1, bh2):
    ...
    """
    M = bh1.m + bh2.m
    x_CM = (bh1.m[:, np.newaxis] * bh1.x + bh2.m[:, np.newaxis] * bh2.x) / M[
        :, np.newaxis
    ]
    v_CM = (bh1.m[:, np.newaxis] * bh1.v + bh2.m[:, np.newaxis] * bh2.v) / M[
        :, np.newaxis
    ]
    bh1.x -= x_CM
    bh1.v -= v_CM
