...
parser = argparse.ArgumentParser(
    description="Extract key quantities from a simulation run for use in later Bayesian hierarchical modelling.",
    allow_abbrev=False,
...
parser.add_argument(
    ...
    "--kickinfo",
    type=str,
    help="path to parameter file describing which kick snapshots to run",
    dest="kpf",
    default=None,
)
parser.add_argument(
    "-m",
    "--method",
    help="method used to generate sample",
    type=str,
    dest="method",
    choices=["mc", "bump", "kick"],
    default="mc",
)
parser.add_argument(
    "-n", "--number", help="perturbation number", dest="pnum", action="append"
)
parser.add_argument(
    "-o", "--overwrite", help="allow overwriting", dest="overwrite", action="store_true"
)
parser.add_argument(
    "-v",
    "--verbosity",
...
class _Extractor:
    ...
    def _make_family_name(self):
        ...
        sim_dir = (
            self.merger_params[0]["calculated"]["full_save_location"]
            .rstrip("/")
            .split("/")[-1]
        )
    ...
    def _create_cube_names(self):
        ...
        """
        d = os.path.join(
            self.analysis_params["file_locations"]["cube_dir"], f"{self.merger_family}"
        )
    ...
    def extract(self, i):
        ...
        except ValueError:  # need to check this
            SL.exception(
                f"UNABLE TO EXTRACT FROM CHILD {self.child_dirs[i]}", exc_info=True
            )
...
class ExtractorMCS(_Extractor):
    def __init__(self, apf, mpf, overwrite=False) -> None:
        ...
        self._child_dirs = [
            os.path.join(run_dir, f"{p}/output") for p in next(os.walk(run_dir))[1]
        ]
...
class ExtractorPS(_Extractor):
    def __init__(self, apf, mpf, overwrite=False, subdir=None) -> None:
        ...
        run_dir = os.path.join(
            self.merger_params[0]["calculated"]["full_save_location"],
            self.merger_params[0]["file_locations"]["perturb_sub_dir"],
        )
        if subdir is None:
            self._child_dirs = [
                os.path.join(run_dir, f"{p}/output") for p in next(os.walk(run_dir))[1]
...
class ExtractorKick(_Extractor):
    def __init__(self, apf, mpf, kickinfo, overwrite=False) -> None:
        ...
        for d in all_dirs:
            if (
                "kick-vel" in d.name
                and run_details["snap_nums"][f"v{d.name.lstrip('kick-vel-')}"]
                is not None
            ):
...
if __name__ == "__main__":
    if args.method == "mc":
        if args.pnum is not None:
            SL.warning(
                "Specific perturbation number analysis is not possible for method type 'mc'!"
            )
        if os.path.isfile(args.mpf):
            ...
            try:
                ...
                SL.exception(
                    "A merger parameter file or a directory of merger parameter files must be specified",
                    exc_info=True,
                )
                raise
    ...
    SL.debug(
        f"Directories to analyse ({len(extractor.child_dirs)}):\n{extractor.child_dirs}"
    )
