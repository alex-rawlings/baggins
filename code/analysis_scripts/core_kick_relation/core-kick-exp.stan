functions {
    vector relation_exp(vector x, real K, real b){
        return K .* pow(x, b) + 1.;
    }

    real relation_exp(real x, real K, real b){
        return K * pow(x, b) + 1.;
    }

    real partial_sum(array[] real y_slice, int start, int end, vector vk, real K, real b, real err){
        return normal_lpdf( y_slice | relation_exp(vk[start:end], K, b), err);
    }
...
generated quantities {
    ...
    vector[N_GQ] rel_mean = relation_exp(vkick_GQ, K, b);
    for(i in 1:N_GQ){
    ...
    for(i in 1:N_tot){
        log_lik[i] = normal_lpdf(rb[i] | relation_exp(vkick[i], K, b), err);
    }
}