...
parser.add_argument(
    type=str, dest="label", help="labelling method", choices=["e", "res"], default="e"
)
parser.add_argument(
    "-d",
    "--dir",
    ...
    default=[],
    dest="extra_dirs",
    help="other HMQ directories to compare",
)
parser.add_argument(
    "-a",
    "--angle",
...
parser.add_argument(
    ...
    type=int,
    dest="orbits",
    help="number of orbits to determine eccentricity over",
    default=5,
)
parser.add_argument(
...
try:
    ...
    SL.exception("Deflection angle must be in range 0<t<180!", exc_info=True)
    raise
...
if args.save_data:
    try:
        ...
        SL.exception(
            f"Data must have equal length arrays, but has lengths {dat_len}!",
            exc_info=True,
        )
        raise
    bgs.utils.save_data(
