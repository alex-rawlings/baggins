...
ra_values = np.full((snapfiles, ), np.nan, dtype=float)
...
for i, snapfile in enumerate(snapfiles):
    ...
    if i == 0:
        ...
        snaptimes = np.full_like(snapfiles, np.nan, dtype=float)
        ra_values = np.full((snapfiles, number_of_systems), np.nan, dtype=float)
...
if args.osipkovmerritt:
    ...
    fig2.savefig(os.path.join(args.figpath, args.figname, "-times"))
plt.show()
