...
parser = argparse.ArgumentParser(
    description="Plot and compare beta(r) profiles for the given snapshots.",
    allow_abbrev=False,
)
parser.add_argument(
    type=str, help="Common path of all snapshots to compare", dest="path"
)
parser.add_argument(
    "-f",
    "--file",
    type=str,
    help="path to snapshot relative to common path",
    dest="files",
    action="append",
)
parser.add_argument(
    "-a", "--all", help="analyse all snapshots in path", dest="all", action="store_true"
)
parser.add_argument(
    "-bw",
    "--binwdith",
    type=float,
    help="fixed bin width to determine beta within",
    dest="binwidth",
    default=0.05,
)
parser.add_argument(
    ...
    dest="osipkovmerritt",
    action="store_true",
    help="fit an Osipkov-Merritt model to the data",
)
parser.add_argument(
    "-fp",
    "--figpath",
...
parser.add_argument(
    ...
    dest="singlesystem",
    action="store_true",
    help="analyse the snapshots as a single system",
)
parser.add_argument(
    "-v",
    "--verbose",
...
for i, snapfile in enumerate(snapfiles):
    ...
    if i == 0:
        ...
        if args.singlesystem or number_of_systems == 1:
            fig, ax = plt.subplots(
                num_rows, 1, sharex="col", squeeze=False, gridspec_kw=gridspec
            )
    ...
    for ind, idx in enumerate(snap.bh["ID"]):
        ...
        if ind == 0:
            s = ax[0][ind].scatter(
                ...
                beta_r,
                s=labsize,
                label="t={:.3f} Gyr".format(snaptime),
                zorder=10,
            )
        else:
            ax[0][ind].scatter(
        ...
        if args.osipkovmerritt:
            ...
            ax[0][ind].plot(
                ...
                alpha=0.8,
                zorder=1,
                label=r"r$_a$: {:.2f}".format(params_opt[0]),
            )
...
if args.verbose:
    print(
        "Plot will have lower y-lim truncated to the lowest {} quantile value determined from all input datasets".format(
            plot_quantile_min
        )
    )
