...
for family, files in stan_files.items():
    figname_base = (
        f"hierarchical_models/merger_time/{args.sample}/{family}/merger-{family}"
    )
    ...
    SL.info(
        f"Number of simulations with usable data (binary): {kepler_model.num_groups}"
    )
    SL.info(
        f"Number of simulations with usable data (hardening): {quinlan_model.num_groups}"
    )

    stan_data = dict(
        N_groups=len(kepler_model.obs["label"]), e_0=kepler_model.obs["e_ini"]
    )
