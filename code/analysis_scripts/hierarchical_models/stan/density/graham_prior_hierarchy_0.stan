...
generated quantities {
    ...
    // sample hyperpriors
    r_b_mean = normal_rng(0, 2.0);
    while(r_b_mean < 0){
        r_b_mean = normal_rng(0, 2.0);
    }

    r_b_std = normal_rng(0, 1.0);
    while(r_b_std < 0){
        r_b_std = normal_rng(0, 1.0);
    }

    Re_mean = normal_rng(0, 10);
    while(Re_mean < 0){
        Re_mean = normal_rng(0, 10);
    }

    Re_std = normal_rng(0, 6);
    while(Re_std < 0){
        Re_std = normal_rng(0, 6);
    }

    log10_I_b_mean = normal_rng(10, 2);
    while(log10_I_b_mean < 4 || log10_I_b_mean > 15){
        log10_I_b_mean = normal_rng(10, 2);
    }

    log10_I_b_std = normal_rng(0, 1);
    while(log10_I_b_std < 0){
        log10_I_b_std = normal_rng(0, 1);
    }

    g_mean = normal_rng(0, 0.5);
    while(g_mean < 0){
        g_mean = normal_rng(0, 0.5);
    }
    ...
    n_mean = normal_rng(0, 8);
    while(n_mean < 0 || n_mean > 20){
        n_mean = normal_rng(0, 8);
    }
    ...
    // sample latent parameters and prior check
    {
        ...
        for(i in 1:N_groups){
            r_b[i] = normal_rng(r_b_mean, r_b_std);
            while(r_b[i] < 0){
                r_b[i] = normal_rng(r_b_mean, r_b_std);
            }

            Re[i] = normal_rng(Re_mean, Re_std);
            while(Re[i] < 0){
                Re[i] = normal_rng(Re_mean, Re_std);
            }

            log10_I_b[i] = normal_rng(log10_I_b_mean, log10_I_b_std);
            while(log10_I_b[i] < 4 || log10_I_b[i] > 15){
                log10_I_b[i] = normal_rng(log10_I_b_mean, log10_I_b_std);
            }
