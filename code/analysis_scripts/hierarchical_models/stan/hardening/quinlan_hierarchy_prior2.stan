...
generated quantities {
    ...
    // K parameter
    K_mean = normal_rng(0, 0.1);
    K_std = normal_rng(0, 0.1);
    while(K_std < 0){
        K_std = normal_rng(0, 0.1);
    }

    // intercept for eccentricity
    e0_mean = normal_rng(e_ini, 0.1);
    while(e0_mean < 0 || e0_mean > 1){
        e0_mean = normal_rng(e_ini, 0.1);
    }
    e0_std = normal_rng(0, 0.1);
    while(e0_std < 0){
        e0_std = normal_rng(0, 0.1);
    }
    ...
    for(i in 1:N_groups){
        ...
        e0[i] = normal_rng(e0_mean, e0_std);
        while(e0[i] < 0 || e0[i] > 1){
            e0[i] = normal_rng(e0_mean, e0_std);
        }
        /*while(e0[i] < 0){
    ...
    for(i in 1:N_tot){
        {
            ...
            e_prior[i] = normal_rng(e_mean_temp, 1e-6);
            //while(e_prior[i] < 0 || e_prior[i] > 1){
            //    e_prior[i] = normal_rng(e_mean_temp, e_err);
            //}
    ...
    /*for(i in 1:N_groups){
        e_prior[new_group_idx[i]] = e0[group_id[i]];
        for(j in new_group_idx[i]+1:new_group_idx[i+1]-1){
            {
                real e_mean_temp = quinlan_e(t[j], t[j-1], K[i], HGp_s[i], inv_a_0[i], e_prior[j-1]);
                e_prior[j] = normal_rng(e_mean_temp, e_err);
                while(e_prior[j] < 0 || e_prior[j] > 1){
                    e_prior[j] = normal_rng(e_mean_temp, e_err);
                }
            }
        }
