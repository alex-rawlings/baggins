...
parser.add_argument("-t", "--thin", type=int, help="thin data", dest="thin", default=100)
...
if args.model == "simple":
    ...
    if args.load_file is not None:
        ...
        # sample
        pq_model = cmf.analysis.PQModelSimple(model_file=stan_model_file, prior_file="stan/peters_quinaln/pq_simple_prior.stan", figname_base=figname_base)
else:
    ...
    if args.load_file is not None:
        ...
        # sample
        pq_model = cmf.analysis.PQModelHierarchy(model_file=stan_model_file, prior_file="stan/peters_quinlan/pq_hierarchy_prior.stan", figname_base=figname_base)
...
if args.prior:
    ...
    pq_model.sample_prior(data=stan_data, sample_kwargs=analysis_params["stan"]["binary_sample_kwargs"])

    fig, ax = plt.subplots(2, 1, sharex="all")
    #ax[1].set_ylim(0,1)
    pq_model.prior_plot(xobs="t", yobs="a", xmodel="t", ymodel="a_prior", ax=ax[0], save=False)
