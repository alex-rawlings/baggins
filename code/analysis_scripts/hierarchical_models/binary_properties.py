...
parser.add_argument(
    "-t", "--thin", type=int, help="thin data", dest="thin", default=None
)
...
try:
    ...
    SL.exception(
        f'There are not enough groups to form a valid hierarchical model. Minimum number of groups is {analysis_params["stan"]["min_num_samples"]}, and we have {kepler_model.num_groups}!',
        exc_info=True,
    )
    raise
...
SL.debug(
    f"Median semimajor axis per group: {[np.nanmedian(g) for g in kepler_model.obs['a']]}"
)
SL.debug(
    f"Median eccentricity per group: {[np.nanmedian(g) for g in kepler_model.obs['e']]}"
)
...
if args.prior:
    ...
    kepler_model.sample_prior(
        sample_kwargs=analysis_params["stan"]["binary_sample_kwargs"]
    )
