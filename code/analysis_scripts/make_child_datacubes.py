...
parser = argparse.ArgumentParser(
    description="Create datacubes of simulation perturbed runs.", allow_abbrev=False
)
...
parser.add_argument(
    "-o", "--over", help="allow overwriting", dest="overwrite", action="store_true"
)
...
if __name__ == "__main__":
    ...
    file_save_dir = os.path.join(
        afv.cube_dir, pfv.full_save_location.rstrip("/").split("/")[-1]
    )
