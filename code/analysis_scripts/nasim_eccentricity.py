...
parser.add_argument("-c", "--cut", type=float, help="eccentricity cut in std.devs.", default=None, dest="ecut")
...
if args.ecut is not None:
    ax.scatter(x, sigma_e_cut, lw=0.5, ec="k", label=f"Sims. ($<{args.ecut:.1f}\\sigma$)", zorder=10, c=sc.get_facecolor(), alpha=0.3, marker="s", s=sc.get_sizes()*1.1)
