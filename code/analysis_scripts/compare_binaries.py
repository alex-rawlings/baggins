...
for j, d in enumerate(ketju_dirs):
    ...
    for i, k in enumerate(ketju_files):
        ...
        if num_dirs == 1:
            ax = cmf.plotting.binary_param_plot(op, ax=ax, label=f"{k.split('/')[-3]}", ls=linestyles[line_count//len(cols)], toffset=toffset)
            if args.orbits:
                for bh in (bh1, bh2):
            ...
            ax = cmf.plotting.binary_param_plot(op, ax=ax, label=(labels[j] if i==0 else ""), c=cols[j], alpha=0.6, markevery=1000, ls=linestyles[line_count//len(cols)], toffset=toffset)
            if args.orbits:
                for bh in (bh1, bh2):
