...
parser.add_argument(
    "-o", "--orbits", action="store_true", dest="orbits", help="plot binary orbits"
)
    ...
    for i, (r, kf) in enumerate(zip(results, kfs)):
        ...
        def _orbit_plotter(bh1, bh2, c):
            ...
            for axidx, q in enumerate((pos, vel)):
                ax2[axidx, 0].plot(
                    q[:, 0], q[:, 2], c=c, alpha=0.7, markevery=[-1], marker="o"
                )
                ax2[axidx, 1].plot(
                    q[:, 0], q[:, 1], c=c, alpha=0.7, markevery=[-1], marker="o"
                )
...
try:
    ...
    except ValueError:  # need to check this error
        ax[0].legend()
    if args.logt:
        ax[0].set_xscale("log")
    if args.save:
