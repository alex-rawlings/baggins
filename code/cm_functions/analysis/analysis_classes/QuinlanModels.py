...
class _QuinlanModelBase(HierarchicalModel_2D):
    def __init__(self, model_file, prior_file, figname_base, num_OOS, rng=None) -> None:
        ...
        self._latent_qtys_labs = [r"$H'(\mathrm{kpc}^{-1} \mathrm{Myr}^{-1})$", r"$\mathrm{kpc}/a_0$", r"$K$", r"$e_0$", r"$\mathrm{err}_{1/a}$", r"$\mathrm{err}_e$"]
    ...
    def all_prior_plots(self, figsize=None):
        ...
        for axi,l in zip(ax, self._folded_qtys_labs):
            axi.set_ylabel(l)
        self.plot_predictive(xobs="t_shift", yobs="inva", xmodel="t", ymodel="inv_a_prior", ax=ax[0], save=False)
        self.plot_predictive(xobs="t_shift", yobs="e", xmodel="t", ymodel="ecc_prior", ax=ax[1], show_legend=False)
    ...
    def determine_merger_timescale_distribution(self, n=1000, save=True):
        ...
        for k in ("a_err", "e_err"): samples.pop(k)
        gq_suffix = "_posterior" if self._fit is not None else ""
        quantiles = self._rng.uniform(size=n)
        for k in samples.keys():
            samples[k] = np.nanquantile(self.sample_generated_quantity(f"{k}{gq_suffix}", state="OOS"), quantiles)
...
class QuinlanModelSimple(_QuinlanModelBase):
    ...
    def all_posterior_pred_plots(self, figsize=None):
        ...
        for axi,l in zip(ax, self._folded_qtys_labs):
            axi.set_ylabel(l)
        self.plot_predictive(xobs="t", yobs="inva", ymodel="inv_a_posterior", ax=ax1[0], save=False)
        self.plot_predictive(xobs="t", yobs="e", ymodel="ecc_posterior", ax=ax1[1], show_legend=False)
