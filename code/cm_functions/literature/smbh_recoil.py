...
def ketju_calculate_bh_merger_remnant_properties(m1, m2, s1, s2, x1, x2, v1, v2):
    ...
    try:
        ...
        _logger.exception(
            "Spin input must have a magnitude less than or equal to 1!", exc_info=True
        )
        raise
    ...
    v_para = (4 * eta) ** 2 * (
        np.dot(Delta_tilde, n0)
        * (
            3678.0
            - 2475.0 * delta_m**2
            + 4962.0 * S0_tilde_para
            + 7170.0 * S0_tilde_para**2
            + 12050.0 * S0_tilde_para**3
        )
        + np.dot(S0_tilde, m59)
        * (
            Delta_tilde_para
    ...
    Ec_para = pow(4 * eta, 2) * (
        E_HU
        + k2a * delta_m * Delta_tilde_para
        + 0.000743 * pow(Delta_tilde_para, 2)
        + k2d * pow(delta_m, 2)
    ...
    # Eq. (34)
    Ec = Ec_para + pow(4 * eta, 2) * (
        pow(S_tilde_ortho, 2) * (e1 + e2 * S_tilde_para + e3 * pow(S_tilde_para, 2))
        + pow(Delta_tilde_ortho, 2)
        * (eps1 + eps2 * S_tilde_para + eps3 * pow(S_tilde_para, 2))
        + pow(delta_m, 2) * pow(S_tilde_ortho, 2) * (E_A + S_tilde_para * E_B)
        + pow(delta_m, 2) * pow(Delta_tilde_ortho, 2) * (E_D + S_tilde_para * E_E)
        + E_F * delta_m * Delta_tilde_ortho * S_tilde_ortho
        + E_G * pow(Delta_tilde_para, 2) * pow(Delta_tilde_ortho, 2)
    ...
    # Eq. (35)
    alpha_align = pow(4 * eta, 2) * (
        L0
        + L1 * S_tilde_para
        + L2a * delta_m * Delta_tilde_para
        + L2b * pow(S_tilde_para, 2)
        + L2c * pow(Delta_tilde_para, 2)
        + L2d * pow(delta_m, 2)
        ...
        + L4c * pow(Delta_tilde_para, 4)
        + L4d * pow(S_tilde_para, 4)
    ...
    # Eq. (36)
    Ac = pow(alpha_align, 2) + pow(4 * eta, 2) * (
        pow(S_tilde_ortho, 2) * (a1 + a2 * S_tilde_para + a3 * pow(S_tilde_para, 2))
        + pow(Delta_tilde_ortho, 2)
        * (zeta1 + zeta2 * S_tilde_para + zeta3 * pow(S_tilde_para, 2))
        + pow(delta_m, 2) * pow(S_tilde_ortho, 2) * (A_A + A_B * S_tilde_para)
        + pow(delta_m, 2) * pow(Delta_tilde_ortho, 2) * (A_D + A_E * S_tilde_para)
        + A_F * delta_m * Delta_tilde_ortho * S_tilde_ortho
        + A_G * pow(Delta_tilde_para, 2) * pow(Delta_tilde_ortho, 2)
