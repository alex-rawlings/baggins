...
class _GalaxyICBase:
    def __init__(self, parameter_file):
        ...
        self.save_location = os.path.join(
            self.parameters["file_locations"]["save_location"], self.name
        )
        self.figure_location = os.path.join(
            self.save_location, self.parameters["file_locations"]["figure_location"]
        )
        self.data_location = os.path.join(
            self.save_location, self.parameters["file_locations"]["data_location"]
        )
        self.seed = self.parameters["general"]["random_seed"]
        self._rng = np.random.default_rng(self.seed)
        self.simulation_start_time = self.parameters["general"][
    ...
    def hubble_redshifted(self):
        ...
        return self.H0_in_Hz * np.sqrt(
            omega_L + omega_M * (1 + self.redshift) ** 3
        )  # in Hz
...
class _StellarComponent(_GalaxyICBase):
    def __init__(self, parameter_file):
        ...
        if self.parameters["stars"]["anisotropy_radius"]["value"] is None:
            ...
            self.anisotropy_radius = self.parameters["stars"]["anisotropy_radius"][
                "value"
            ]
...
class _StellarCore(_StellarComponent):
    def __init__(self, parameter_file):
        ...
        self.to_kpc_units()
        rhof = lambda r: r**2 * Terzic05(
            r,
            rhob=self.core_density * 1e9,
            rb=self.core_radius,
            n=self.sersic_index,
            g=self.core_slope,
            b=self.sersic_b_parameter,
            Re=self.effective_radius,
            a=self.transition_index,
        )
        self.total_mass = (
            4
            * np.pi
            * self.mass_light_ratio
            * scipy.integrate.quad(rhof, 1e-5, self.maximum_radius, limit=100)[0]
        )
...
class _DMHaloNFW(_DMComponent):
    ...
    def virial_radius(self):
        ...
        return (
            self.virial_mass
            / (self.overdensity * 4 * np.pi / 3 * self.critical_density)
        ) ** (1 / 3)
        ...
        if isinstance(val, str) or val is None:
            ...
            if valid_str:
                ...
                spin_mag = scipy.stats.beta.rvs(
                    *bh_spin_params.values(), random_state=self._rng
                )
                ...
                _logger.warning(
                    "Invalid spin distribution parameters given: setting spin to [0,0,0]"
                )
