...
class GalaxyIC(_GalaxyICBase):
    def __init__(self, parameter_file, stellar_mass=None, sahu_legacy=False):
        ...
        if star_pars["particle_mass"]["value"] is not None:
            if star_pars["use_cored"]:
                ...
                # save the kpc values
                self._calc_quants["input_effective_radius"] = {
                    "value": self.stars.effective_radius,
                    "unit": self.stars.stellar_distance_units,
                }
                self._calc_quants["input_core_radius"] = {
                    "value": self.stars.core_radius,
                    "unit": self.stars.stellar_distance_units,
                }
            else:
        ...
        if dm_pars["particle_mass"]["value"] is not None:
            try:
                ...
                if stellar_mass is None:
                    _logger.exception(
                        "If no stellar component supplied, a stellar mass must be given to initialise the DM halo",
                        exc_info=True,
                    )
                    raise
                else:
            ...
            if dm_pars["use_NFW"]:
                self.dm = _DMHaloNFW(
                    stellar_mass=_star_mass, parameter_file=parameter_file
                )
            else:
                self.dm = _DMHaloDehnen(
                    stellar_mass=_star_mass, parameter_file=parameter_file
                )
        ...
        if bh_pars["set_spin"] is not None:
            try:
                ...
                if stellar_mass is None:
                    _logger.exception(
                        "If no stellar component supplied, a stellar mass must be given to initialise the SMBH mass",
                        exc_info=True,
                    )
                    raise
                else:
    ...
    def plot_mass_scaling_relations(self):
        ...
        halo_mass_seq, moster_seq = Moster10(
            self.stars.total_mass, [1e10, 1e15], z=self.redshift, plotting=True
        )
        ax[2].plot(
            halo_mass_seq, moster_seq - halo_mass_seq, label="Moster+10", color=cols[0]
        )
        halo_mass_seq, girelli_seq = Girelli20(
            self.stars.total_mass, [1e10, 1e15], z=self.redshift, plotting=True
        )
        ax[2].plot(
            halo_mass_seq,
            girelli_seq - halo_mass_seq,
            label="Girelli+20",
            color=cols[1],
        )
        halo_mass_seq, behroozi_seq = Behroozi19(
            self.stars.total_mass, [1e10, 1e15], z=self.redshift, plotting=True
        )
        ax[2].plot(
            halo_mass_seq,
            behroozi_seq - halo_mass_seq,
            label="Behroozi+19",
            color=cols[2],
        )
        ax[2].scatter(
            self.dm.log_peak_mass,
            self.stars.log_total_mass - self.dm.log_peak_mass,
            color=cols[3],
            zorder=10,
            label="Simulation",
        )
    ...
    def plot_dm_cut_function(self, cut_params):
        ...
        div99 = (cut_params["shift"] - np.log(1 / (1 - 0.99) - 1)) / cut_params[
            "slope"
        ]  # where the function reaches 0.99
    ...
    def generate_galaxy(self, allow_overwrite=False):
        ...
        try:
            ...
            _logger.exception(
                f"File {self.hdf5_file_name} already exists!", exc_info=True
            )
            raise
        ...
        if self.stars is not None:
            ...
            if isinstance(self.stars, _StellarCore):
                ...
                rho = (
                    lambda r: 0.1
                    * self.stars.mass_light_ratio
                    * Terzic05(r, **df_kwargs)
                )
                star_distribution = mg.GenericSphericalComponent(
                    density_function=rho,
                    particle_mass=self.stars.particle_mass,
                    particle_type=mg.ParticleType.STARS,
                )
                self._calc_quants["stars"]["actual_total_mass"] = (
                    star_distribution.mass * 1e10
                )
            else:
                star_distribution = mg.DehnenSphere(
                    self.stars.total_mass,
                    scale_radius=self.stars.scale_radius,
                    gamma=self.stars.gamma,
                    particle_mass=self.stars.particle_mass,
                    particle_type=mg.ParticleType.STARS,
                )
        ...
        if self.dm is not None:
            ...
            if isinstance(self.dm, _DMHaloNFW):
                if self.parameters["dm"]["NFW"]["cut_pars"] is not None:
                    ...
                    cut_params = dict(slope=1.0, approx0=1e-5, max_scale_radius=20.0)
                    _logger.warning("Using default NFW cut parameters")
                dm_distribution = mg.NFWSphere(
                    Mvir=self.dm.peak_mass,
                    particle_mass=self.dm.particle_mass,
                    particle_type=mg.ParticleType.DM_HALO,
                    z=self.redshift,
                    use_cut=True,
                    cut_params=cut_params,
                )
                self._calc_quants["dm"]["actial_total_mass"] = (
                    dm_distribution.mass * 1e10
                )
                self._calc_quants["dm"]["concentration"] = self.dm.concentration
                self.plot_dm_cut_function(cut_params=cut_params)
            else:
                dm_distribution = mg.DehnenSphere(
                    mass=self.dm.peak_mass,
                    scale_radius=self.dm.scale_radius,
                    gamma=self.dm.gamma,
                    particle_mass=self.dm.particle_mass,
                    particle_type=mg.ParticleType.DM_HALO,
                )
        ...
        if self.bh is not None:
            # set up bh
            bh_particle = mg.CentralPointMass(
                mass=self.bh.mass,
                softening=self.bh.softening,
                chi=self.bh.spin,
                particle_type=mg.ParticleType.BH,
            )
        ...
        # clean centre
        if self.bh is not None:
            generated_galaxy = mg.TransformedSystem(
                generated_galaxy,
                mg.FilterParticlesBoundToCentralMass(
                    central_object_mass=self.bh.mass,
                    minimum_semi_major_axis=self.minimum_radius,
                ),
            )
        ...
        mg.write_hdf5_ic_file(
            self.hdf5_file_name, generated_galaxy, center_CoM=self.recentre_to_com
        )
    ...
    def plot_ic_kinematics(self, num_rots=3):
        ...
        mass_centre = pygad.analysis.shrinking_sphere(
            ic.stars, pygad.analysis.center_of_mass(ic.stars), 25.0
        )
        ...
        radial_surf_dens = dict(
            stars=pygad.analysis.profile_dens(
                ic.stars,
                qty="mass",
                r_edges=radial_bin_edges["stars"],
                center=mass_centre,
            ),
            dm=pygad.analysis.profile_dens(
                ic.dm, qty="mass", r_edges=radial_bin_edges["dm"], center=mass_centre
            ),
            stars_dm=pygad.analysis.profile_dens(
                ic, qty="mass", r_edges=radial_bin_edges["stars_dm"], center=mass_centre
            ),
        )
        ...
        ax[0, 0].plot(
            radial_bin_centres["stars"],
            radial_surf_dens["stars"],
            color=cols[3],
            lw=5,
            alpha=0.6,
        )
        if isinstance(self.stars, _StellarCusp):
            dehnen_params_fitted = fit_Dehnen_profile(
                radial_bin_centres["stars"],
                radial_surf_dens["stars"],
                total_stellar_mass,
                bounds=([1, 0.5], [20, 2]),
            ...
            ax[0, 0].plot(
                ...
                Dehnen(
                    radial_bin_centres["stars"],
                    *dehnen_params_fitted,
                    total_stellar_mass,
                ),
                color=cols[1],
                label=r"a:{:.1f}, $\gamma$:{:.1f}".format(*dehnen_params_fitted),
            )
            ax[0, 0].legend()
        ...
        if isinstance(self.dm, _DMHaloDehnen):
            dehnen_params_fitted = fit_Dehnen_profile(
                radial_bin_centres["dm"], radial_surf_dens["dm"], total_dm_mass
            )
            ax[0, 1].plot(
                radial_bin_centres["dm"],
                Dehnen(radial_bin_centres["dm"], *dehnen_params_fitted, total_dm_mass),
                color=cols[1],
                label=r"a:{:.1f}, $\gamma$:{:.1f}".format(*dehnen_params_fitted),
            )
            ax[0, 1].legend()
        ...
        ax[0, 2].plot(
            radial_bin_centres["stars_dm"],
            radial_surf_dens["stars_dm"],
            color=cols[3],
            lw=5,
            alpha=0.6,
        )
        if isinstance(self.stars, _StellarCusp) and isinstance(self.dm, _DMHaloDehnen):
            dehnen_params_fitted = fit_Dehnen_profile(
                radial_bin_centres["stars_dm"],
                radial_surf_dens["stars_dm"],
                total_stellar_mass + total_dm_mass + ic.bh["mass"],
            )
            ax[0, 2].plot(
                radial_bin_centres["stars_dm"],
                Dehnen(
        ...
        if isinstance(self.stars, _StellarCusp):
            ...
            self._calc_quants["kinematics"]["half_mass_radius_analytic"] = {
                "unit": "kpc",
                "value": hmr,
            }
            ax[1, 0].scatter(
                np.log10(hmr),
                np.log10(np.unique(ic.stars["mass"]) * len(ic.stars["mass"])),
                color=cols[3],
                marker="x",
                s=60,
                zorder=10,
        ...
        logRe_vals = np.log10(
            bulgeBHData.table["Re_maj"].astype("float")
            * bulgeBHData.table["scale"].astype("float")
        )
        ax[1, 0].errorbar(
            logRe_vals,
            bulgeBHData.table.loc[:, "logM*_sph"],
            yerr=bulgeBHData.table.loc[:, "logM*_sph_ERR"],
            marker=".",
            ls="None",
            elinewidth=0.5,
            capsize=0,
        ...
        self._calc_quants["kinematics"]["half_mass_radius_true"] = {
            "unit": "kpc",
            "value": hmrT,
        }
        ...
        self._calc_quants["stars"]["radius_to"] = {
            "unit": "kpc",
            "inner_100": i100star,
            "inner_1000": i1000star,
        }
        ...
        ax[2, 1].scatter(
            np.log10(LOS_sigma), np.log10(ic.bh["mass"]), zorder=10, color=cols[3]
        )
        ax[2, 1].errorbar(
            BHsigmaData.table.loc[:, "logsigma"],
            BHsigmaData.table.loc[:, "logBHMass"],
            xerr=BHsigmaData.table.loc[:, "e_logsigma"],
            yerr=[
                BHsigmaData.table.loc[:, "e_logBHMass"],
                BHsigmaData.table.loc[:, "E_logBHMass"],
            ],
            marker=".",
            ls="None",
        ...
        if self.parameters["bh"]["spin_relation"] is not None:
            if self.bh.spin_relation == "zlochower_dry":
                ...
                _logger.error(
                    "Invalid BH spin parameters provided, no distribution will be plotted."
                )
        else:
        ...
        ax[2, 2].scatter(
            bhspin_mag, bh_chi_dist.pdf(bhspin_mag), color=cols[3], zorder=10
        )
