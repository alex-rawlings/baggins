data {
    ...
    //vector of initial eccentricity
    //vector[N_child] e0;
}
...
transformed data {
    ...
    temp_integrated_a[2:N_tot] = -log(fabs(inv_a[1] ./ inv_a[2:N_tot]));
...
transformed parameters {
    ...
    //temp_integrated_a[1] = 0.0;
    //temp_integrated_a[2:N_tot] = -log(fabs(inv_a_true[1] ./ inv_a_true[2:N_tot]));

    delta_e_true = K[child_id] .* temp_integrated_a;
...
model {
    ...
    // likelihood
    inv_a ~ normal(inv_a_true, sigma_inv_a);
   
    delta_e ~ normal(delta_e_true, sigma_e);
}
...
generated quantities {
    ...
    vector[N_tot] temp_integrated_a_posterior;
    vector[N_tot] delta_e_posterior;

    // perform posterior predictive check //
    // for inverse semimajor axis
    inv_a_posterior = normal_rng(HGp_s_mu_scaled, HGp_s_tau_scaled) * t_transformed + normal_rng(c_mu, c_tau);

    // integrate
    temp_integrated_a_posterior[2:N_tot] = -log(fabs(inv_a_posterior[1] ./ inv_a_posterior[2:N_tot]));


     //delta_e_posterior = normal_rng(K_mu, K_tau) * normal_rng(HGp_s_mu_scaled, HGp_s_tau_scaled) * temp_integrated_a;
     delta_e_posterior = normal_rng(K_mu, K_tau) * temp_integrated_a;
