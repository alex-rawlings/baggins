...
radial_bin_centres = bgs.mathematics.get_histogram_bin_centres(radial_bins)
...
def extract_data(pickle_file, a_val=10):
    ...
    for j, bhfile in enumerate(bhfiles):
        ...
        snaplist = bgs.utils.get_snapshots_in_dir(os.path.join(data_path, f"{j:03d}/output"))
        snap_idx = bgs.analysis.snap_num_for_time(snaplist, t)
        snap = pygad.Snapshot(snaplist[snap_idx], physical=True)
        eff_rad, vsigRe, vsigr, _Sigma = bgs.analysis.projected_quantities(snap, obs=3, r_edges=radial_bins)
        ...
        snap.delete_blocks()
        del snap
    bgs.utils.save_data(dd, pickle_file)
...
#else:
#    obs = bgs.utils.load_data(args.file)


if args.load_file is not None:
    my_stan = bgs.analysis.StanModel.load_fit(args.load_file, figname_base=figname_base)
else:
    my_stan = bgs.analysis.StanModel(model_file="stan/graham.stan", prior_file="stan/graham_prior.stan", figname_base=figname_base)
...
observations = bgs.utils.load_data(obs_file)
