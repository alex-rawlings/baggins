...
peritime, idx, radial_sep = bgs.analysis.find_pericentre_time(bh1, bh2, return_sep=True)

snap_files = bgs.utils.get_snapshots_in_dir(data_path)
...
for i, snapfile in enumerate(snap_files):
    ...
    times[i] = bgs.general.convert_gadget_time(snap)*1e3
    bhseps[i] = np.linalg.norm(snap.bh["pos"][0,:] - snap.bh["pos"][1,:])

snapNum = bgs.analysis.snap_num_for_time(snap_files, peritime[0])
...
bgs.plotting.shade_bool_regions(ax, bh1.t/myr, orbit_energy<0, alpha=0.4, color="tab:red")
