...
if False:
    ...
    hmqfiles = bgs.utils.get_files_in_dir(main_path)

    # note the argument order here is different to the function 
    # core_Sersic_profile
    log_core_sersic = lambda x, rb, Re, Ib, gamma, n, a: np.log10(bgs.literature.core_Sersic_profile(x, Re=Re, rb=rb, Ib=Ib, n=n, gamma=gamma, alpha=a))
    ...
    for i, f in enumerate(hmqfiles):
        hmq = bgs.analysis.HMQuantitiesSingleData.load_from_file(f)
        ...
        print(popt)
    data = {"vk":vk, "rb":rb}
    bgs.utils.save_data(data, "data.pickle")
else:
    data = bgs.utils.load_data("data.pickle")
    vk = data["vk"]
    rb = data["rb"]
...
axt = bgs.plotting.twin_axes_from_samples(ax, vk, vk*vesc)
...
plt.legend()
#bgs.plotting.savefig("vk-rb.png")
plt.show()
