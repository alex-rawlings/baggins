...
snapdirs = [d.path for d in os.scandir(basedir) if "kick-vel-0" in d.name and "0960" not in d.name]
snapdirs.sort()
...
if extract_data:
    ...
    bgs.utils.save_data(data, "bound.pickle")
else:
    data = bgs.utils.load_data("bound.pickle")
...
cmapper, sm = bgs.plotting.create_normed_colours(0, 900)

marker_lims = [np.inf, -np.inf]
for k, v in data.items():
    if k == "__githash" or k == "__script":
        continue
    ax.scatter(v["times"], v["sep"], color=cmapper(float(k[1:])), marker="o", s=v["num_bound_stars"]/1e3, ec="k", lw=0.5, zorder=2)
    ax.plot(v["times"], v["sep"], color=cmapper(float(k[1:])), zorder=0)
...
for msize in np.arange(*marker_lims):
    msize = 10**msize
    ax.scatter([], [], s=msize/1e3, label=int(msize), c="gray")
ax.set_xlim(0, 0.25)
