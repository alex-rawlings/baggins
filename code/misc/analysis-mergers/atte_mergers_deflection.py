...
for i, d in enumerate(data_dirs):
    ...
    for run_num in range(5):
        ...
        ketju_file = cmf.utils.get_ketjubhs_in_dir(data_dir)[0]

        # get the BH particles
        bh1, bh2, merged = cmf.analysis.get_bound_binary(ketju_file)
        orbit_pars = ketjugw.orbital_parameters(bh1, bh2)
        bh1_pb, bh2_pb, bound_state = cmf.analysis.get_binary_before_bound(ketju_file)
        if bound_state == "oscillate":
            print("Binary is not reliably bound, skipping")
            continue

        # pericentre deflection angle before binary is bound
        bh1_pb, bh2_pb = cmf.analysis.move_to_centre_of_mass(bh1_pb, bh2_pb)
        try:
            _, peri_idxs = cmf.analysis.find_pericentre_time(bh1_pb, bh2_pb, prominence=0.005)
            prebound_deflection_angles = cmf.analysis.deflection_angle(bh1_pb, bh2_pb, peri_idxs)
        except:
        ...
        thetas[run_num] = cmf.analysis.first_major_deflection_angle(prebound_deflection_angles, np.pi/6)[0]
        eccs[run_num] = np.nanmedian(ecc)
        ax2.plot(orbit_pars["t"][:ecc_idx]/cmf.general.units.Myr, ecc, c=cols[i])
    ax1.scatter(np.degrees(thetas), eccs, label=os.path.basename(d).split("_")[-3])
ax1.legend()
for axi in ax1, ax2:
    axi.set_ylim(0,1)
cmf.plotting.savefig(os.path.join(cmf.FIGDIR, "deflection_angles/atte_minor_mergers_theta"), fig=fig1)
cmf.plotting.savefig(os.path.join(cmf.FIGDIR, "deflection_angles/atte_minor_mergers_ecc"), fig=fig2)
plt.show()
