...
for i, snapfile in enumerate(snaplist):
    ...
    g_rho_sigma["t"][i] = cmf.general.convert_gadget_time(snap, "Myr")
    g_rho_sigma["Gps"][i] = cmf.analysis.get_G_rho_per_sigma(snaplist, g_rho_sigma["t"][i], extent=bh_binary.r_infl)
    del snap 

plt.plot(g_rho_sigma["t"], g_rho_sigma["Gps"], "-o")
plt.axvline(bh_binary.r_hard_time, c="tab:red")
plt.scatter(bh_binary.r_hard_time, cmf.analysis.get_G_rho_per_sigma(snaplist, bh_binary.r_hard_time, extent=bh_binary.r_infl))
plt.xlabel("t/Myr")
