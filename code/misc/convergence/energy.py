...
for pfR, n, col in zip(("DE-030-0005r1.py", "DE-030-0005r2.py", "DE-030-0005r5.py"), ("1", "2", "5"), ("tab:blue", "tab:orange", "tab:green")):
    ...
    if len(snaplist) > 1:
        snap_perturb_idx = cmf.analysis.snap_num_for_time(snaplist, pfv.perturbTime*1e3)
    else:
        snap_perturb_idx = -1
    snap = pygad.Snapshot(snaplist[snap_perturb_idx], physical=True)
    total_energy_p = cmf.analysis.calculate_Hamiltonian(snap.stars)
    ...
    for i in range(10):
        ...
        total_energy = cmf.analysis.calculate_Hamiltonian(snap.stars)
