...
parser.add_argument("-b", "--batch", type=int, help="create a batch of merger ICs", dest="batch", default=1)
        ...
        for i, s1 in enumerate(suffixes[:args.batch], start=1):
            for s2 in suffixes[i:args.batch]:
                ...
                with open(new_filename, "r+") as f:
                    ...
                    contents, sc = re.subn(f"  random_seed:.*", f"  random_seed: {str(rng.integers(100000))}", contents, flags=re.MULTILINE)
                    try:
                        assert sc == 1
                    except AssertionError:
                    ...
                    for k, v in zip(("  galaxy_name_1:", "  galaxy_name_2:"), (s1, s2)):
                        ...
                        contents, sc = re.subn(f"{k}.*", f"{k} {gal_name}", contents, flags=re.MULTILINE)
                        try:
                            assert sc == 1
                        except AssertionError:
