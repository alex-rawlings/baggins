...
parser = argparse.ArgumentParser("Compare BH positions and velocities between different ICs", allow_abbrev=False)
...
for d, label in zip(ic_dirs, labels):
    ...
    for f in ic_files:
        ...
        for i, bhid in enumerate(snap.bh["ID"]):
            ...
            ax[0,i].plot(snap.bh[id_mask]["pos"][0,0], snap.bh[id_mask]["pos"][0,2], marker="o", ls="", label=label)
            ax[1,i].plot(snap.bh[id_mask]["vel"][0,0], snap.bh[id_mask]["vel"][0,2], marker="o", ls="")
