...
parser.add_argument(
    ...
    "--quantilecut",
    dest="quantile_cut",
    help="remove data that is above this \nradial quantile",
    default=1,
)
parser.add_argument(
    "-p",
    "--percentages",
    dest="percentages_to_check",
    help="find the radius corresponding to these percentages of mass (can be specified multiple times)",
    action="append",
    default=None,
    type=float,
)
...
if args.verbose:
    print(
        "Stability checked at radii enclosing mass percentages: {}".format(
            args.percentages_to_check
        )
    )
...
for ind, col in enumerate(args.percentages_to_check):
    stability_data.insert(
        1 + ind * 2, "stars_" + str(col), np.full_like(datfiles, np.nan)
    )
...
for ind, this_file in enumerate(datfiles):
    ...
    if "stars" in gal:
        ...
        mass_centre = pygad.analysis.shrinking_sphere(
            gal.stars, pygad.analysis.center_of_mass(gal.stars), 25.0
        )
    if "dm" in gal:
        particle_families.append("dm")
        if "stars" not in gal:
            mass_centre = pygad.analysis.shrinking_sphere(
                gal.dm, pygad.analysis.center_of_mass(gal.dm), 500
            )
    for p_ind, particle_type in enumerate(particle_families):
        ...
        if args.quantile_cut < 1:
            ...
            subsnap = subsnap[
                subsnap["r"]
                < pygad.UnitScalar(
                    np.quantile(subsnap["r"], args.quantile_cut), units="kpc"
                )
            ]
        ...
        for f_ind, percentage in enumerate(args.percentages_to_check):
            ...
            stability_data.loc[ind, particle_type + "_" + str(percentage)] = float(
...
with open((outpath + "/" + pfv.galaxyName + "_stability.txt"), "w") as f:
    ...
    for ind, col in enumerate(stability_data.columns[1:], start=1):
        ...
        ax.plot(
            stability_data.loc[:, "Time"],
            stability_data.loc[:, col],
            c=colour_value,
            label=(label_value if not label_flag else ""),
        )
        ax.scatter(
            stability_data.iloc[-1, 0],
            stability_data.iloc[-1, ind],
            c=colour_value,
            marker=point_style[particle],
            zorder=10,
        )
        f.write(
            "{:s},{:s},{:.3f}\n".format(
                ...
                label_value,
                (
                    (stability_data.iloc[-1, ind] - stability_data.iloc[0, ind])
                    / stability_data.iloc[0, ind]
                    * 100
                ),
            )
        )
