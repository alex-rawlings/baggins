...
\begin{table}
    \centering
    \caption{The parameters for each hyperparameter distribution for the core-S\'ersic model, where each distribution is a normal distribution.}
    \label{tab:cshypers}
    \begin{tabular}{ccc}
        \hline
        Hyperparameter & $\mu$ & $\sigma$ \\
        \hline
        $r_{\mathrm{b}, \mu}$ & 0.0 & 2.0 \\
        $r_{\mathrm{b}, \sigma}$ & 0.0 & 1.0 \\
        $R_{\mathrm{e}, \mu}$ & 0.0 & 14.0 \\
        $R_{\mathrm{e}, \sigma}$ & 0.0 & 6.0 \\
        $\log_{10}\Sigma_{\mathrm{b}, \mu}$ & 0.0 & 20.0 \\
        $\log_{10}\Sigma_{\mathrm{b}, \sigma}$ & 0.0 & 10.0 \\
        $\gamma_\mu$ & 0.0 & 0.5 \\
...
The hierarchical model takes three observable quantities, denoted by hat-variables in circles in \autoref{fig:graphicalmodel}. 
...
There are six latent parameters ($r_\mathrm{b}$, $R_\mathrm{e}$, $\log_{10}\Sigma_\mathrm{b}$, $\gamma$, $n$, and $\alpha$, the set of which we denote as $\kappa$) that describe each \child{} simulation in a family. 
These latent parameters, depicted as non-hatted variables in circles in \autoref{fig:graphicalmodel}, are sampled to obtain a latent estimate on the projected mass density, $\Sigma$.
The latent parameters in the set $\kappa$ are strictly non-negative, with the exception of $\log_{10}\Sigma_\mathrm{b}$. 
...
The sampled hyperparameter distributions are shown as corner plots in Figures \ref{fig:corner_params_1}-\ref{fig:corner_params_3}. \drafting{Corner plots maybe not so informative here, do we really expect correlations? Maybe 1D distributions better for the hyperpriors.}
...
For each radial bin, we backwards-model and na\"ively bin our thirty observations of $\log_{10}\hat{\Sigma}$ for that radial bin to assess the likely distribution.
We find that the different projections of the mass density profile in a given radial bin are well-described by a normal distribution in logarithm space. 
Thus, as likelihood function, we use the normal distribution $\mathrm{Lognorm}(\Sigma, \hat{\sigma}_\Sigma^2)$.

\drafting{Write explicitly the posterior distribution we are sampling from.}

\begin{figure*}
    \includegraphics{{graham_density-H-H-3.0-0.001-hierarchy_posterior_pred_log10_proj_density_mean}.pdf}
    \caption{\drafting{Posterior predictive estimate for the core-S\'ersic fit for highest (fiducial) mass resolution. Darker shaded regions correspond to greater Highest Density Intervals (HDIs). The observed data is overlaid, and seen to lie well within the 50\% HDIs. }}
    \label{fig:postpred}
\end{figure*}

\begin{figure*}
    \includegraphics{{graham_density-H-H-3.0-0.001-hierarchy_latentqty_compare}.pdf}
...
For comparison, we also show the estimated uncertainty on each latent parameter determined using na\"ive $\chi^2$ minimisation techniques.
...
The $\chi^2$ minimisation fails to capture the correct estimates for the latent parameter estimates, with some parameters (e.g. $n$) converging to the boundary value we limit the Least Squares method to (for $n$, the bounds are $[0,20]$).
...
To extract the data for our hierarchical model for hardening rate, we take for each \child{} simulation the semimajor axis values between $a=a_\mathrm{h}$ and $a/\mathrm{pc}=10$.
