...
\begin{equation}
f(r) = \frac{\alpha}{1 + \exp[(r_\mathrm{c}-r)/d_\mathrm{c}]},
\end{equation}
...
We set the scaling factor $\alpha=0.5$, and use cut-off scales $r_\mathrm{c}=2 a_\mathrm{h}$
...
\begin{equation}
\vb{a}_{\mathrm{DF}} = 2 f(|\vb{x}|) \vb{a}_{1,\mathrm{DF}}.
\end{equation}
...
\begin{itemize}
\item scaling of the model?
\item initial condition: specify impact parameter, initial velocity and separation. Like the radial trajectories in the sims.
\item solving numerically
\end{itemize}
