...
\begin{figure*}
    ...
    \caption{Top panel shows the equivalent \drafting{(better term? Reciprocal?)} eccentricity $1-e$ as a function of time $t$ for select representative simulations from the $e_0 = 0.90$ set (solid lines) and $e_0 = 0.99$ set (dashed lines). Time is shown on a logarithmic scale to highlight the variability in eccentricity prior to the SMBH binary becoming bound (vertical black lines). The equivalent eccentricity for a full set of a given mass resolution is shown in the bottom left panel for the $e_0 = 0.90$ suite and in the bottom right panel for the $e_0 = 0.99$ suite as a function of $t' = t-t(a=a_\mathrm{h})$ for five orbits centred about $t'=0$. The dashed line in the bottom panels indicates the collective mean of the equivalent eccentricities averaged over the span of the respective line. The blue area corresponds to the $2.5\sigma$ scatter in the data. Note the asymmetry in the grey area is due to the logarithmic scale of the $y$-axis: the standard deviation is by construct symmetric about the mean. A zoom inset in the bottom right figure shows the tight bunching of $1-e$ for this particular simulation set. In both the shown $e_0 = 0.90$ and $e_0 = 0.99$ sets, one simulation has a low eccentricity, displacing it to a value of $1-e$ greater than $2.5\sigma$ from the collective mean.}
...
An representative set of realisations for the $e_0=0.90$ orbit is shown in the bottom left panel of \autoref{fig:eccentricities}.
...
\begin{figure*}
    ...
    \caption{
    Scattering deflection angle $\theta$ \drafting{update to $\theta_\mathrm{defl}$} and the resulting binary eccentricity
