...
To understand the dependency of the binary eccentricity on the 
deflection angle $\theta_\mathrm{defl}$ during the binary formation,
we have constructed a simple toy model that can reproduce the observed
...
For simplicity, we consider only equal-mass mergers, and take the BH orbits
...
\end{equation}
where $\vb{x}=(x,y)$ is the separation vector of the BHs,
$M_\bullet = 10^8\,\Msun$ is the mass of a single BH,
...
with its long axis aligned along the $x$-axis.
The acceleration from this potential is \citep[e.g.][]{binney2008}
\begin{equation}
\vb{a}_\mathrm{bg} = - 2 \pi G \rho \operatorname{diag}(A_x,A_y) \vb{x},
\end{equation}
where $A$ coefficients are related to the eccentricity $e_\mathrm{s}$ of the spheroid as
\begin{align}
...
\end{align}
The stellar density is set to the value $\rho = 300 \,\Msun\,\mathrm{pc}^{-3}$,
...
and $X = |\vb{v}_1|/\sqrt{2}\sigma_\mathrm{M}$,
where the stellar velocity dispersion is taken to have the value
$\sigma_\mathrm{M}=200\,\mathrm{km\,s^{-1}}$
to approximately match the simulations.
The Coulomb logarithm is taken to have the value $\ln{\Lambda} = 5$,
which is appropriate for the size of the simulated systems.

The effect of dynamical friction weakens as the BH binary orbit shrinks,
and to account for this we multiply the acceleration given by Eq.~\eqref{eq:chandra_df}
with the smooth cut-off function
\begin{equation}
f(a) = \frac{1}{1 + \exp[(a_\mathrm{c}-a)/d_\mathrm{c}]}.
\end{equation}
Here $a$ is the semi-major axis of a bound binary, and for unbound binaries we set $f(a)=1$.
We use the cut-off scales $a_\mathrm{c}=2 a_\mathrm{h}$
