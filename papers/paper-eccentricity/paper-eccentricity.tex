...
We present our findings first for the SMBH binary eccentricity scatter for initially low-eccentricity orbits (\autoref{ssec:lowe}), and then present the findings for initially high-eccentricity orbits (\autoref{ssec:highe}).
We explain the results using an analytical representation in \autoref{ssec:toy}.
...
\begin{figure*}
    \includegraphics{theta_e_sim_and_model_090}
    \includegraphics{theta_e_sim_and_model_099}
    \caption{
    ...
    compared to reasonably well fitting model curves (see \autoref{ssec:toy}).
    The model curve for the $e_0=0.90$ case has been shifted left by $12\degr$
    to better match the simulation data.
    }
    \label{fig:theta_e}
\end{figure*}
...
We compute the resulting eccentricities for a range of impact parameters for the two initial velocities.

To compare the model against the simulations,
we use the deflection angle in the first pericentre passage instead of the impact parameter,
as it can be more robustly measured from the simulation data.
The deflection angle is given by \citep{binney2008}:
\begin{equation}
...
binary angular momentum vector and $E$ the binary orbital energy at the time of the pericentre passage.
To measure this angle from the simulations,
we set as the first `hard-scattering' angle $\theta_\mathrm{defl}$ the first 
...
This corresponds to the second or third actual pericentre passage in the simulations.
In the model calculations we simply use the first actual pericentre for this calculation.
...
and the required slight shift could be due to the galactic potential being slightly tilted compared to the
BH trajectories due to the complex dynamics at this stage.
In the $e_0=0.99$ case we do not shift the model curve, although here a shift of a few degrees might give a better
fit to the location of the minimum at $\theta \approx 100\degr$.
In this case we can also see that the simulation data has more scatter around
...
\drafting{(Move these bits to the right place)}
