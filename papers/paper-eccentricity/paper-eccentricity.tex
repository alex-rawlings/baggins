...
\begin{figure*}
    ...
    \caption{Equivalent eccentricity $1-e$ as a function of time $t'$ for a reprsentative set of simulations with $e_0=0.90$ (left) and $e_0 = 0.90$ (right). \drafting{Shift data so $t'$ is defined such that $t'=0$ when $t=t_h$}. The dashed line indicates the collective mean of the equivalent eccentricities averaged over the span of the respective line. The grey area corresponds to $1\sigma$ scatter in the data \drafting{change to $2.5\sigma$}. Note the asymmetry in the grey area is due to the logarithmic scale of the $y$-axis: the standard deviation is by construct symmetric about the mean.}
...
An representative set of realisations for the $e_0=0.90$ orbit is shown in the left panel of \autoref{fig:eccentricities}.
...
One simulation is seen to lie beyond $2.5\sigma$ of the mean $1-e$ line.
...
Similar to the $e_0=0.90$ case, we show a representative set of realisation for the $e_0=0.99$ orbit in the right panel of \autoref{fig:eccentricities}.
...
We find that the scaling of $\sigma_e$ is steeper than the $1/\sqrt{M_\bullet/m_\star}\propto1/\sqrt{N_\star}$ relation described in \citet{nasim2020}.
...
\drafting{
Explain the deflection angle and eccentricity stuff seen in the simulations here,
then the next section just focuses on the toy model.
}

\drafting{(These paragraphs just copypasted, were mixed in the toy model discussion. Rewrite as appropriate)}
...
We set as the first `hard-scattering' angle the first 
angle $\theta_\mathrm{defl}$ that exceeds an orbit-dependent threshold:
$\theta_{\mathrm{defl,min}}=90\degr$ for $e_0=0.99$ and $\theta_{\mathrm{defl,min}}=45\degr$ for $e_0=0.90$.
In general, this corresponds to the second or third actual pericentre passage of the SMBHs in the simulations.
An example of $\theta_\mathrm{defl}$ is shown for two realisations of the $e_0=0.99$ orbit in \autoref{fig:orbit}, which occurs between the points A and B in the SMBH binary orbit in each inset panel. 
We observe evidence for a relationship between the deflection angle $\theta_\mathrm{defl}$ and the resulting eccentricity at the time the SMBH binary becomes hard, which we aim to explain using a simple toy model.

\begin{figure}
    \includegraphics{orbit}
    \caption{Orbits from $t=0\,\mathrm{Myr}$ to $t=t_\mathrm{bound}$ of a single SMBH in two realisations of the $e_0=0.99$ mergers, which by symmetry of the equal mass system is the same as the second BH orbit reflected. Each line is coloured according to time of the simulation, however the colour for $t<22\,\mathrm{Myr}$ is constant for visual clarity of the later evolution. In each inset panel, point A shows a time just before the first hard scattering $\theta_\mathrm{defl}$, and point B a time just after. The circle marker indicates the final position of the SMBH before the SMBHs form a bound binary.}
    \label{fig:orbit}
\end{figure}
...
\begin{figure*}
    ...
    \caption{
    Scattering deflection angle $\theta$ and the resulting binary eccentricity
...
\subsection{Reproducing the eccentricity scatter with a toy model}\label{ssec:toy}
...
BH binary binding phase reasonably well,
...
We use the error controlled 8th order Runge--Kutta method DOP853 included in the SciPy library \citep{virtanen2020}
for this purpose.
...
but by testing different values we find that $e_\mathrm{s}=0.905$ gives behaviour
that closely resembles the simulation results.
...
In the model, a $90\degr$ deflection during the first pericenter passage
...
In addition to the reference $e_\mathrm{s}=0.905$ model, the figure also shows the model results for
$e_\mathrm{s}=0.900$ and $e_\mathrm{s}=0.910$, as particularly the $e_0=0.99$ case is quite sensitive to this parameter.
...
the model curve in the $\theta \gtrsim 120\degr$ region,
...
Overall we can however see that this simple model correctly captures the behaviour of eccentricity seen in the simulations.
