...
In agreement with previous studies \citep{nasim2021b} \drafting{others?}, we find an increase in the core radius $\rb$ with kick velocity, however only up to a given kick velocity.
We find that unlike the results of \citet{nasim2021b}, where core radius monotonically increases with kick velocity, we observe a peak in core radius occurring at $480\,\kmps$, with the core radii measured from simulations with kick velocities in excess of $480\,\kmps$ plateauing at $\rb\sim1.75 r_{\mathrm{b},0}$.
...
We fit a function that is the sum of a modified $t$-distribution with a sigmoid function, and takes the form:
...
The fit is plotted with an orange line in \autoref{fig:vkrb}, and the values of the fitted parameters are given in \autoref{tab:vkrb}.
