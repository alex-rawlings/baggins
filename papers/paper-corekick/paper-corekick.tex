...
\begin{figure}
    ...
    \caption{
        ...
        All simulations with $\vk \geq 0.5v_\mathrm{esc}=900\,\kmps$ share the same colour, as there is little variation in the profiles for these kick velocities. 
        A clear dichotomy is revealed in the mass profiles, whereby high kick velocoties induce a core, and low kick velocities do not, with a sudden transition at \drafting{$\vk=600\,\kmps$}.
        }
    \label{fig:density3d}
\end{figure}
...
For the $\vk=0\,\kmps$ case, we determine the SMBH influence radius to be $r_\mathrm{infl}\simeq 1.5\,\mathrm{kpc}$, and is insensitive to the time of measurement.
In \autoref{fig:density3d}, we observe a clear dichotomy in the 3D spatial distribution of stellar mass, with mergers that had an SMBH with a kick velocity $\vk<540\,\kmps$ having more mass in the central $3\,\mathrm{kpc}$ ($2r_\mathrm{infl}$) than those mergers with $\vk>540\,\kmps$. 
All merger remnants have a consistent mass density distribution at $r>3\,\mathrm{kpc}$. 
Additionally, for kick velocities $\vk<540\,\kmps$, we observe a gradient in decreasing central stellar density with increasing kick velocity.
We check the time dependence of this dichotomy by constructing the 3D stellar mass distributions at additional times $t=0.056\,\mathrm{Gyr}$, $t=0.094\,\mathrm{Gyr}$, and $t=0.12\,\mathrm{Gyr}$, and find the dichotomy persists, with the same kick velocity split.
...
Our results indicate that a kick non-zero kick velocity always serves to enlarge the core region relative to the core formed during the SMBH binary scouring phase.
In agreement with previous studies \citep{gualandris2008,nasim2021b}, we find an increase in the core radius $\rb$ with kick velocity, however only up to a given kick velocity, in agreement with the results of \citet{boylan2004}.
We find that unlike the results of \citet{nasim2021b}, where core radius monotonically increases with kick velocity, we observe a peak in core radius occurring at $\vk=480\,\kmps=0.27\,v_\mathrm{esc}$, with the core radii measured from simulations with kick velocities in excess of $480\,\kmps$ plateauing at $\rb\sim1.7 r_{\mathrm{b},0}$.
We caution however that the systems in the study of \citet{nasim2021b} differ from ours in both structure, and the time of the analysis.
Whereas we use a fixed time following the SMBH binary coalescence for our analysis, \citet{nasim2021b} wait until either the ejected SMBH settles back to the CoM, or $2\,\mathrm{Gyr}$ has passed.
\drafting{As discussed in section \textbf{Atte}, we find relaxation increasingly dominates our merger remnants with increasing time.}
As a result, a direct comparison between this work and that of \citet{nasim2021b} cannot be made.

Complimentary to the peak in core radius at $480\,\kmps$, we observe a dip in the projected density at the core radius $\Sigma_\mathrm{b}$ at this same kick velocity in \autoref{fig:csparams}.
The effective radius $\Reff$ and core density slope $\gamma$ are relatively robust to the SMBH recoil velocity.
...
\begin{figure}
    ...
    \caption{
        Bayesian estimate of the merger remnant core size $r_b$, scaled to the core size of the pre-merger remnant $r_{\mathrm{b},0}$, as a function of kick velocity $\vk$.
...
\drafting{Would a Gaussian process fit be better motivated here?}
To predict the distribution of core sizes, we require a mapping from the kick velocity to the core radius. 
Previous work by \citet{nasim2021b} fit a power law of the form:
\begin{equation}\label{eq:vkrb_nasim}
    \frac{\rb}{r_{\mathrm{b},0}} = K v'^\beta + 1
\end{equation}
where $v'=\vk/v_\mathrm{esc}$, and $K$ and $\beta$ are free parameters.
We see immediately that \autoref{eq:vkrb_nasim} does not capture the peak in the data at $480\kmps$, and so opt for an alternative form.
We fit a function that is the sum of a modified Student's $t$-distribution with a sigmoid function, and takes the form:
\begin{equation}\label{eq:vkrb}
    \frac{\rb}{r_{\mathrm{b},0}} = \frac{A(v'; \mu,\nu,\sigma)+B(v'; \mu,\nu,\sigma)}{2\pi\sigma} - S(v';k,b,\mu) + c,
\end{equation}
where
\begin{equation}
    A(v'; \mu,\nu,\sigma) = \sqrt{\frac{\nu-2}{\nu}} \left(1 + \left(\frac{v' - \mu}{\sigma \sqrt{\frac{\nu-2}{\nu}}} \right)^2 \right)^{-(\nu + 1)/2},
\end{equation}
and
\begin{equation}
    B(v'; \mu,\nu,\sigma) = \sqrt{\frac{\nu}{\nu-2}} \left(1 + \left(\frac{v' - \mu}{\sigma \sqrt{\frac{\nu}{\nu-2}}} \right)^2 \right)^{-(\nu + 1)/2},
\end{equation}
and
\begin{equation}
    S(v';k,b,\mu) = \frac{k}{1 + \exp(-b (v'-\mu))}.
\end{equation}
Hence \autoref{eq:vkrb} takes six free parameters ($\mu$, $\nu$, $\sigma$, $k$, $b$, $c$), which we constrain using the median trend of the $vk/v_\mathrm{esc}$--$\rb$ relation in \autoref{fig:vkrb}, and assume a normally-distributed scatter $\tau$.
The fit is found using the same Bayesian methods as in \autoref{ssec:projdens} (including posterior checking), and is plotted with an orange line in \autoref{fig:vkrb}, with the values of the fitted parameters given in \autoref{tab:vkrb}.

\begin{table}
    \caption{Values of the fitted parameters in the core radius -- kick velocity relation in \autoref{eq:vkrb}.}
    \label{tab:vkrb}
    \begin{tabular}{lccc}
        \hline
        Parameter & Median & IQR & Constraints \\
        \hline
        $\mu$ & 0.182 & 0.0427 & $\geq 0$ \\
        $\nu$ & 21.5 & 11.9 & $\geq 0$ \\
        $\sigma$ & 0.601 & 0.298 & $\geq 0$ \\
        $k$ & 0.894 & 0.189 & $\geq 0$ \\
        $b$ & -34.5 & 30.9 & $\leq 0$ \\
        $c$ & 1.64 & 0.0661 & $\geq 0$ \\
        $\tau$ & 0.170 & 0.0336 & $\geq 0$ \\
        \hline
    \end{tabular}
\end{table}
...
Using transformation sampling allows us to obtain a distribution of core radii predicted by a given kick velocity model, as shown in \autoref{fig:rb_pdf}, whilst simultaneously accounting for uncertainty in the fit described by \autoref{eq:vkrb}.
We find that the mode of the forward-folded core radius distribution is $1.07\,\mathrm{kpc}$ ($1.69\,r_{\mathrm{b},0}$).
Thus, assuming the \citet{zlochower2015} model to be a reasonable description of SMBH recoil velocities, a large fraction massive elliptical galaxies should have a non-negligible core \citep{ferrarese1994,savorgnan2016,sahu2019}. 


\begin{figure}
    \centering
    \includegraphics[width=0.4\textwidth]{rb_pdf}
    \caption{Probability density function of transformation sampled core radius. The SMBH kick velocity is Monte Carlo sampled from the \citet{zlochower2015} relation assuming randomly-aligned azimuthal spins, and pushed through the fitted model in \autoref{eq:vkrb}. The predicted kick velocities range from $\vk=0\,\kmps$ to $\vk\sim4000\,\kmps$.
    The asymmetry at $\rb/r_{\mathrm{b},0}\simeq 1$ is due to the low kick velocities producing a modest core, as seen in \autoref{fig:vkrb}.}
    \label{fig:rb_pdf}
\end{figure}
...
\end{equation}
As shown in \autoref{fig:h4}, for the parallel projections, simulations with $\vk \geq 600\,\kmps$ tend to positive values of $\langle h_4 \rangle$, asymptotically reaching $\langle h_4 \rangle \sim 1$.
Conversely, when the recoiling SMBH has a kick velocity $\vk < 600\,\kmps$, the final value of $\langle h_4 \rangle$ is evenly distributed about $\langle h_4 \rangle = 0$: there is no systematic preference to heaver or weaker tails in the LOS velocity distribution.

Conversely, for the orthogonal projections, we observe a dichotomy similar to that seen in \autoref{fig:density3d}, with the bifurcation occurring at a similar kick velocity, here $600\,\kmps$; both partitions tend to a positive value of $\langle h_4 \rangle$. 
Within the two partitions of the bifurcation, there is no apparent dependence on kick velocity.
The gradient between the two sets differs most noticeably in the radial range $2  \leq R/\mathrm{kpc} < 3$.
This can be understood as those recoiling SMBHs with $\vk \geq 600\,\kmps$ having a larger $h_4$ magnitude per Voronoi bin than the $\vk < 600\,\kmps$ simulations.
...
\begin{figure}
    ...
    \caption{
        ...
        Lines terminating with a circle point indicate simulations with $\vk/\kmps < 600$, those with square points indicate simulations with $600 \leq \vk/\kmps < 1200$, those with triangle points $1200 \leq \vk/\kmps < 1800$, and those with diamond points $\vk/\kmps > 1800$.
        For the parallel case, no systematic trend between $h_4$ and $\vk$ appears to exist.
        Conversely, for the orthogonal projection, a dichotomy similar to the bifurcation seen in \autoref{fig:density3d} is observed. 
    }
    \label{fig:h4}
\end{figure}
...
\begin{figure*}
    ...
    \caption{
        ...
        Note the colour scheme is consistent with that in \autoref{fig:density3d}, with kick velocities exceeding $900\,\kmps$ represented using the same colour.
    }
    \label{fig:orbits}
\end{figure*}
...
The first and most important, is the complete absence of rosette orbits for all simulations with $\vk>600\,\kmps$.
Conversely, all simulations with $\vk<600\,\kmps$ (except for $\vk=540\,\kmps$) have more than $\sim25\%$ of the orbits in the innermost radial bin being identified as rosette. 
The second and third trend are complimentary, and are an increase in the fraction of $\pi$-box orbits with increasing kick velocity, and a similar decrease in the fraction of $x$-tube orbits with increasing kick velocity.
Finally, there is a weak tendency for simulations with $\vk<900\,\kmps$ to have slightly higher fractions of boxlet orbits at small radii, and higher fractions of $z$-tube orbits, than those simulations with $\vk>900\,\kmps$.
...
We take two representative recoil velocities, $\vk=0\,\kmps$ and $\vk=1500\,\kmps$, and limit our investigation to stellar particles in a rectangular box defined by $0\leq x/\mathrm{kpc}$, $-5\leq y/\mathrm{kpc}\leq 5$, and $-5\leq z/\mathrm{kpc}\leq 5$ (approximately matching the aperture of the mock IFU maps), recalling that the SMBH is ejected along the positive $x$ axis.
We create velocity vector fields of the stellar particle motion in the $x$-$y$ plane \autoref{fig:wake}.
...
The IFU maps describe the line of sight velocity distribution, thus the motion along the $x$-axis in \autoref{fig:wake} is equivalent to the parallel projection, and the $y$-axis equivalent to the orthogonal projection previously described in \autoref{ssec:ifu}.
...
\section{Specific IFU maps}
\drafting{Here we include both paralle and orthogonal projections of the IFU maps for three representative kick velocities: $\vk=0\,\kmps$, $\vk=600\,\kmps$, and $\vk=1800\,\kmps$.}
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{IFU_para_v0600}
    \caption{
        ...
        Most striking is the concentration of negative ordered velocity $V$ slightly offset from the remnant centre.
        The other kinematic moments display a similar spatial offset.
    }
    \label{fig:IFU600p}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{IFU_ortho_v0600}
    \caption{
        The same as \autoref{fig:IFU600p}, but in the orthogonal projection. 
        Note the colour scaling is consistent with \autoref{fig:IFU0p}.
        Compared to the parallel case, the ordered velocity $V$ and $h_3$ moments appear consistent with the $\vk=0,\kmps$ case, however a spatial offset from the centre persists in $\sigma$ and $h_4$.
    }
    \label{fig:IFU600o}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{IFU_para_v1800}
    \caption{
        Mock integral field unit kinematic maps of the $\vk=1800\,\kmps$ remnant in the parallel projection, plotted out to $0.25\,\Reff$ in a circular aperture. 
        Note the colour scaling is consistent with \autoref{fig:IFU0p}.
        This projection displays clear ordered rotation, however there is no clear peak in velocity dispersion.
        The other kinematic moments are quantitatively similar to the $\vk=0,\kmps$ case. 
    }
    \label{fig:IFU1800p}
\end{figure*}
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{IFU_ortho_v1800}
    \caption{
        The same as \autoref{fig:IFU1800p}, but in the orthogonal projection. 
        Note the colour scaling is consistent with \autoref{fig:IFU0p}.
        Aside from the lack of ordered rotation, there is little difference between the orthogonal and parallel projections at this kick velocity. 
    }
    \label{fig:IFU180o}
\end{figure*}

\section{Stellar velocity fields}
We create vector fields of stellar particles for two representative merger remnants, namely $\vk=0\,\kmps$ and $\vk=1500\,\kmps$.
The velocity fields are created for particles in a rectangular box defined by $0\leq x/\mathrm{kpc}$, $-5\leq y/\mathrm{kpc}\leq 5$, and $-5\leq z/\mathrm{kpc}\leq 5$, as described in \autoref{sec:discussion}, and are shown in \drafting{figure}.

\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{wake}
    \caption{
        Stellar velocity field for $\vk=0\,\kmps$ (left panel) and $\vk=1500\,\kmps$ (right panel), for an aperture that is approximately consistent with the IFU maps presented in e.g. \autoref{fig:IFU0p}. 
        The right panel demonstrates the stellar particles streaming into the potential wake caused by the recoiling SMBH (the SMBH is beyond the extent of the plot), whereas the same motion is not present in the left panel.
        This causes a difference in the LOSVDs of the two systems, noticeably the $h_4$ kinematic moment.
    }
    \label{fig:wake}
\end{figure}
