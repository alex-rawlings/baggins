...
\drafting{Need to check if others discuss this:} We find that the core radius saturates to a value of $\sim2$ times that of the merger remnant with no kick velocity by $\vk = 600\,\kmps$, corresponding to half of the escape velocity of the merger remnant core. 
...
\end{equation}
to our simulations, which is thus valid for $0\leq\vk/\kmps\leq900$, or equivalently $0\leq\vk/v_\mathrm{esc}\leq 0.75$.
\drafting{We estimate the parameters $K$ and $\beta$ using HMC, and find median values of $K=$ and $\beta=$.}

\drafting{Using \autoref{eq:vkrb}, we push through the distribution of kick velocities from \citet{zlochower2015} for dry mergers assuming random azimuthal alignment (their Fig. 15, right panel.) 
This allows us to obtain a distribution of core radius values predicted by a given kick velocity model.
We find\dots}


\begin{table}
    \caption{Hyperparameter distributions for core-S\'ersic model.}
    \label{tab:hyper}
    \begin{tabular}{llc}
        \hline
...
\begin{figure*}
    ...
    \caption{
        ...
        Stellar particles \drafting{currently it's just a subset} are assigned to one of seven different orbital families, and binned into ten logarithmically-spaced radial shells such that $0.2 \leq R/\mathrm{kpc} \leq 100$.
...
We perform an orbit analysis on the sixteen merger remnants in our sample following \citet{frigo2021}.
...
The potential of the merger remnant is checked for stability, and each \drafting{currently a subset} stellar particle is integrated for fifty orbits to determine which, if any, orbital resonances exist. 

As seen in \autoref{fig:orbits}, we observe a clear trend of higher kick velocity remnants having a higher fraction of $\pi$-box orbits, and a lower fraction of $x$-tube orbits, than their lower kick velocity counterparts.
The other five orbital families are generally consistent between all merger remnants. 
...
\drafting{Why are the $\langle h_4 \rangle$ profiles consistent in shape though??}

\drafting{
    Might move this to discussion, just put here for now to get the idea down.
    z-tubes not primary contributor to LOS velocity (rotation about z axis).
    Higher kick correlated with reduction in x-tubes (orbits about the x-axis), increase in pi-box.
    Pi-box show no resonant motion (no net ang. mom.)\dots here's the leap: reduced resonant motion seen as a reduction in `symmetric deviations' in line profile?
    Also look directly at kinematic maps: 0 kick case. Dominated by x tubes at radii > 1kpc, small region of negative h4 extends to here before becoming positive. 
}
...
\begin{figure*}
    ...
    \caption{
        ...
        \drafting{TODO: maybe change km/s to $\kmps$}
    }
    \label{fig:orbits2}
\end{figure*}
