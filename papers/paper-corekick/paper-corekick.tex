...
We estimate the parameters $K$ and $\beta$ using HMC, and find median values of $K=2.830$ and $\beta=0.558$.
...
The dimensionless spin parameter for the dry mergers follows a beta distribution, namely:
\begin{equation}
    P_\mathrm{Zlochower}(\alpha) \propto (1-\alpha)^{4.66884-1} \alpha^{10.5868-1}
\end{equation}
This allows us to obtain a distribution of core radius values predicted by a given kick velocity model, as shown in \autoref{fig:rb_pdf}.
We find that the mode of the forward-folded core radius distribution is $1.61\,\mathrm{kpc}$ ($3.06\,r_{\mathrm{b},0}$).
