...
Those simulations where the maximum displacement of the kicked SMBH exceeds \drafting{$30\,\mathrm{kpc}$ check} are not included in the analysis, as we expect that observationally a SMBH further than this distance from the centre of its host galaxy would not be identified with that galaxy.
In practice, this limits our sample of analysed simulations to those with $\vk\leq 960\kmps$, thus totalling seventeen simulations in our analysis.
...
We use the Hamiltonian Monte Carlo code \textsc{Stan} with four chains each of 2000 posterior sample draws (excluding 1000 burn-in draws) to fit the model parameters, ensuring that:
\begin{enumerate}
...
\begin{equation}
    \langle h_4 \rangle = \frac{\langle R h_4 \rangle}{\langle R \sqrt{V^2+\sigma^2}\rangle},
\end{equation}
...
\begin{figure*}
    ...
    \caption{
        Mock integral field unit kinematic maps of the $\vk=960\,\kmps$ remnant, again plotted out to $0.5\,\Reff$. 
...
\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{corner_latent_960}
    \caption{
        Corner plot of the latent parameters in the core-Se\'rsic model for the $\vk=960\kmps$ instance.
