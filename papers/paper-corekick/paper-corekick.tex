...
We see immediately that \autoref{eq:vkrb_nasim} does not capture the peak in the data at \drafting{$600\kmps$}, and so opt for an alternative form.
...
\end{equation}
Hence \autoref{eq:vkrb} takes six free parameters ($\mu$, $\nu$, $\sigma$, $k$, $b$, $c$), which we constrain using the median trend of the $vk/v_\mathrm{esc}$--$\rb$ relation in \autoref{fig:vkrb}, and plot with an orange line. 
