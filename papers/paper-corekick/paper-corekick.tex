...
For the parallel projections, we do not observe a systematic variation in $h_4$ with increasing kick velocity.
...
\end{equation}
As shown in \autoref{fig:h4}, for the parallel projections, we do not observe a systematic variation in $h_4$ with increasing kick velocity.
Conversely, for the orthogonal projections, we observe a dichotomy similar to that seen in \autoref{fig:density3d}, with the bifurcation occurring at a similar kick velocity, here $600\,\kmps$. 
...
Rosette orbits are bounded by distinct radial bounds \citep[pericentre and apocentre, e.g.][]{binney2008}, and produce a distinctive ring pattern in the $h_4$ kinematic moment, whereby a torus of negative $h_4$ separates an inner and outer region of positive $h_4$, as seen in fig. 14 of \citet{frigo2021}.
...
We can gain insight into the probability of finding a stellar particle on a radial orbit by approximating the motion as a simple harmonic oscillator normalised by some dispersion measure $\xi$. 
The radial motion is described by a Gaussian function of $r/\xi$ about the centre of the potential, and thus the probability is proportional to $\int_r ^{r+\dd{r}} |\exp(-(r/\xi)^2)|^2\dd{r}$.
The integral of a Gaussian function results in the error function, and thus the probability of finding a stellar particle in the interval $r+\dd{r}$ is near zero for small $r/\xi$, and rapidly grows as $r/\xi \rightarrow 1$, before plateauing as $r/\xi \rightarrow \infty$.
...
We see the manifestation of an increase in radially-biased orbits in the three dimensional stellar density profile, where the bifurcation in the density profiles occur at the same kick velocity as when $\pi$-box and boxlet orbits start to dominate the inner orbital structure.
...
\begin{figure}
    ...
    \caption{
        ...
        \drafting{Update colour bar to match other plots}
    }
    \label{fig:triax}
\end{figure}
