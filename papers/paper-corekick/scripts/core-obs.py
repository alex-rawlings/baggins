...
# add an inset axis
axins = ax.inset_axes([0.07, 0.65, 0.4, 0.3], xlim=(5.75e9, 6.3e9), ylim=(0.5, 0.95), xticklabels=[], yticklabels=[], xticks=[], yticks=[])
...
m87_mask = np.logical_and(data1.table.loc[:, "BH_mass"]>6e9, data1.table.loc[:, "BH_mass"]<6.2e9)
axins.annotate(r"$\mathrm{M87}$", (data1.table.loc[m87_mask, "BH_mass"], data1.table.loc[m87_mask, "core_radius_kpc"]+0.02), (6.1e9, 0.85), arrowprops={"arrowstyle":"wedge", "fc":"k"})
...
xhat = np.geomspace(data1.table.loc[:, "BH_mass"].min(), data1.table.loc[:, "BH_mass"].max(), 1000)
slope = 1.17
intercept = 10.27
...
for axi in (ax, axins):
    axi.fill_between(xhat, 10**(mean_relation - intrinsic_scatter), 10**(mean_relation + intrinsic_scatter), alpha=0.3, fc=line1[-1].get_color(), zorder=1)
...
for axi in (ax, axins):
    for i, (vk, rb) in enumerate(kicks.items()):
        axi.scatter(my_BH_mass, rb, marker="s", color=vkcols.get_colour(float(vk.lstrip("v"))), label=(r"$\mathrm{Simulation}$" if i==0 else ""), zorder=2.5, **figure_config.marker_kwargs)
