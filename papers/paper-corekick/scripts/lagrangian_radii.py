...
def get_lagrangian_radii_evolution(v_kick):
    ...
    r_mBH = data[:,4] #radius enclosing stellar particles with mass equal to the BH mass 
    sigma = data[:,5]
...
v_list = [0, 60,120, 180,  240,  300,  360, 420, 480,
           540, 600, 660, 720, 780, 840, 900, 960, 1020, 2000]
...
cp1 = ConnectionPatch(xyA=(100,0.998), xyB=(0, 0), axesA=ax, axesB=axins, coordsA="data", coordsB="axes fraction", alpha=0.5, zorder=4.99)
cp2 = ConnectionPatch(xyA=(100,1.45), xyB=(0, 1), axesA=ax, axesB=axins, coordsA="data", coordsB="axes fraction", alpha=0.5, zorder=4.99)
ax.add_patch(cp1)
ax.add_patch(cp2)
...
for i in range(len(v_list)):
    ...
    r_lag=r_smooth
    ax.plot((t[t<=t_settle]-t_merge)*1000, r_lag[t<=t_settle], '-', color=vkcols.get_colour(v ))
    axins.plot((t[t<=t_settle]-t_merge)*1000, r_lag[t<=t_settle], '-', color=vkcols.get_colour(v ))
...
if log:
    ax.set_xscale('log')
    plt.savefig('lagrangian_radii_log.pdf')
else:
