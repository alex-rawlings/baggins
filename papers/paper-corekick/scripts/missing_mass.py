...
def missing_mass_plot(filename, nro_iter=10000, min_r=1e-2, ax=None):
    ...
    for j, v in enumerate(vkicks):
        ...
        for i in tqdm(range(nro_iter), desc=f"Missing mass for case {v}"):
            ...
            m, abserr = integrate.quad(mass_deficit, min_r, rb_new, args=(rb_new, Re_new, n_new, log10densb_new, g_new, a_new))
            while np.isnan(m):
                ...
                m, abserr = integrate.quad(mass_deficit, min_r, rb_new,
    ...
    bp = ax.boxplot([_v/norm_val for _v in mdef_dict.values()], positions=velocities, showfliers=False,
