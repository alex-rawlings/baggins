...
parser = argparse.ArgumentParser(description="Plot orbit families, based on `script_freq.py`", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    for i in np.arange(nbin)+1:
        ...
        if rad_len[-1]>0:
            for cl in possibleclasses:
                classfrequency[i-1,cl] = float(len(radclassids[radclassids==cl]))/rad_len[-1]
        else:
            SL.debug("Warning: no particles in current radial bin")
    rad_len = np.array(rad_len)

    if returnextra:
        return meanrads, classfrequency, rad_len, b92class, pericenter, apocenter, minangmom
    else:
