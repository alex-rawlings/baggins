...
parser.add_argument(
    ...
    dest="threshold",
    type=float,
    help="Minimum detection distance threshold",
    default=5,
)
parser.add_argument(
    "-v",
...
print(
    f"{frac_above_X*100:.3f}% of sampled apocentres are above {args.threshold:.2f}kpc"
)
frac_above_X_proj = gp.fraction_apo_above_threshold(args.threshold, proj=True)
print(
    f"{frac_above_X_proj*100:.3f}% of sampled projected apocentres are above {args.threshold:.2f}kpc"
)
