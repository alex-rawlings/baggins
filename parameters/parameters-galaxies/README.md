# Description of the parameter file format for isolated galaxies
...
- `logStellarMass`: log10 of total stellar mass (**SD**)  
...
- `DM_cut`: dict of parameters to be parsed to the merger generator to cut the diverging D profile (**NFW**). Dict keys are:
    - `slope`: the 'steepness' of the sigmoid function
    - `approx0`: a function value approximately equal to 0
    - `scaled_max_radius`: where the function value should be equal to *approx0*, in terms of the virial radius
...
- `BH_spin_from`: distribution to generate SMBH spin from
- `real_BH_mass`: actual SMBH mass if observed (not critical to pipeline)  

## Data Files  
- `bulgeBHData`: path to observational data of BH-bulge mass relation  
- `massData`: path to observational data of ETG masses  
- `BHsigmaData`: path to observational data of BH-sigma relation  
- `fDMData`: path to observational data of inner dark matter fraction - Re relation
...
- `count_STARS`: number of stellar particles in model  
- `count_DM_HALO`: number of DM particles in model  
- `number_ketju_particles`: estimated number of Ketju particles in the galaxy  
- `count_BH`: number of SMBH particles in model (should be 1 generally)  
- `halfMassRadius`: radius within which half the mass is located (**SD**)  
- `projected_half_mass_radius`: the projected half mass radius from 3 different viewing angles  
- `inner_DM_fraction`: fraction of DM within projectedHalfMassRadius  
- `virial_radius`: the measured virial radius  
- `virial_mass`: the measured virial mass  
